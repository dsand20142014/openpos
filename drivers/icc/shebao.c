...
unsigned short uiSHEBAOTimeoutCount;
...
void SHEBAO_RecvCharProc(unsigned char ucReader,unsigned char ucCh)
{
    ...
    case ICCSESSION_WARMRESET:
        SHEBAO_ResetRecvCharProc(ucReader,ucCh);
        break;
    case ICCSESSION_DATAEXCH:
        SHEBAO_APDUExchRecvCharProc(ucReader,ucCh);
        return;
    default:
        break;
    }
...
void SHEBAO_TimeoutProc(unsigned char ucReader,unsigned char ucTimeoutType)
{
    ...
    case ICCSESSION_COLDRESET:
    case ICCSESSION_WARMRESET:
        SHEBAO_ResetTimeoutProc(ucReader,ucTimeoutType);
        break;
    default:
        break;
    }
...
void SHEBAO_ResetTimeoutProc(unsigned char ucReader,unsigned char ucTimeoutType)
{
    ...
    {
	case 1:
		uiSHEBAOTimeoutCount ++;
		if( uiSHEBAOTimeoutCount >= 105 )
		{
...
//			HALICC_EnableRecv(ucReader);
	        ...
	        HALICC_SetTimer(ucReader,0x65,0x40,0x17,0x02,1);
...
unsigned char SHEBAO_T0SendReceive(unsigned char ucReader)
{
    ...
    unsigned char ucCh,ucLen;
    unsigned short uiI,uiStart;
                ...
                {   /*EN: data from card */
                    ...
                    for(uiI=uiAPDURecvedLen;uiI<(pCurrICCInfo->aucAPDUSendBuf[4]+2);uiI++)
                    {
                        ...
                        if( !ucStatus )
                        {
                            ...
                            if( (uiI-uiStart)==1 )
                            {
                            	if( (pCurrICCInfo->aucAPDURecvBuf[uiStart]==0x6A)&&
                            	 	(pCurrICCInfo->aucAPDURecvBuf[uiI]==0x83)
                            	 )
                            		break;
                            }
                        }else
