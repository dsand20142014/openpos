...
extern void QINGD_TimeoutProc(unsigned char ucReader, 
                              unsigned char ucTimeoutType);
...
unsigned char ATR_CheckFinished(unsigned char *pucATR,unsigned short ucATRLen)
{
    ...
    if ( ucATRLen >= 2 ) {
        ...
        do {
            ...
            {
                if ( uiATRTotalLen == ucATRLen+1 )   return(LIBICCERR_ATRHASLASTCHAR);
                else   return(LIBICCERR_ATRNOTFINISHED);

            } else {
                if (  (uiATRTotalLen == ucATRLen )  && ( !ucNextTDnExist ) )
                    return(LIBICCERR_SUCCESS);
                else  if ( ucNextTDnExist ) {
                    uiI += ucTCharNB;
                    ucTDIndex ++;
                } else
                    if ( uiATRTotalLen > ucATRLen )    return(LIBICCERR_ATRTOOLONG);
            }
...
unsigned char ATR_Process(unsigned char ucReader)
{
    ...
    unsigned char ucT0;
    unsigned char ucTA1;
    unsigned char ucTA2 = 0;
    unsigned char ucTC1;
    unsigned char ucTC2;
    unsigned char ucTC3;
    unsigned char ucTB3;
    unsigned char ucTA3;
    unsigned char ucTD1 = 0;
    unsigned char ucTD2;
