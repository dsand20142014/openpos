...
DRVOUT	OSICCDrvOut;
DRVOUT	OSICCDrvOut2 ;
DRV_MMAP *myOSICCDrv = NULL;//用于直接访问
int fd_icc;
...
void OSICC_Init(void)
{
   ...
   // munmap(ic_drvout_data, 1024);
    ...
    fd_icc = open("/dev/TDA8007", O_RDWR);
    if (fd_icc<0)
    {
	...
	myOSICCDrv = (DRV_MMAP*)malloc(sizeof(DRV_MMAP));
      memset(myOSICCDrv, 0, sizeof(DRV_MMAP));
    }
}
...
#if 0
    Uart_Printf("\r\n***************------%s------ ************************\r\n", __func__);
    Uart_Printf("OSICCDrvOut2.ucReqNR=%x ucGenStatus=%x  ucDrvStatus=%x\n",
        OSICCDrvOut.ucReqNR, OSICCDrvOut.ucGenStatus, OSICCDrvOut.ucDrvStatus);

    for (i=0; i<OSICCDrvOut.aucData[0]+1; i++)
        Uart_Printf("%02x ", OSICCDrvOut.aucData[i]);
    Uart_Printf("\n");
    Uart_Printf("******==========%s=============***************\n", __func__);
#endif
}
...
void OSICC_DrvRun(void)
{
   ...
   // OSDRV_Wait(&OSICCDrvOut2);
	//printf("\r\n134\r\n");
    //zengshu.2011.7
   // memcpy(&OSICCDrvOut2,&(myOSICCDrv->pDrvOut),sizeof(DRVOUT));
	//printf("\r\n137\r\n");
...
DRV_OUT *OSICC_Insert(void)
{
    ...
    OSICC_DrvActive();
//	OSICC_DrvRun();
  //  printf("(DRV_OUT *)&myOSICCDrv->pDrvOut:\r\n",(DRV_OUT *)&myOSICCDrv->pDrvOut);
...
ICC_ANSWER *OSICC_Command(unsigned char ucReader, ICC_ORDER *pOrder)
{
    ...
    ICCReport.Len = 0;
#ifdef DBG
    Uart_Printf("=============>1 %s pICCOrder->order=%x, ucReader=%x\n", __func__, pICCOrder->order, ucReader);
#endif
...
#ifdef DBG
    Uart_Printf("=============>2 %s pICCOrder->order=%x, ucReader=%x\n", __func__, pICCOrder->order, ucReader);
#endif
...
#ifdef DBG
    Uart_Printf("=============>3 %s pICCOrder->order=%x, ucReader=%x\n", __func__, pICCOrder->order, ucReader);
#endif
