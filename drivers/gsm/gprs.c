...
#define GSM_PWR_ON			_IOW(GSM_IOC_MAGIC,1, int)	//上电
#define GSM_PWR_OFF			_IOW(GSM_IOC_MAGIC,2, int)	//断电
#define GSM_SOFTONOFF_ON	_IOW(GSM_IOC_MAGIC,3, int)	//ON/OFF脚拉高
#define GSM_SOFTONOFF_OFF	_IOW(GSM_IOC_MAGIC,4, int)	//ON/OFF脚拉低
#define GSM_RST_ON			_IOW(GSM_IOC_MAGIC,5, int)	//RST脚拉高
#define GSM_RST_OFF			_IOW(GSM_IOC_MAGIC,6, int)	//RST脚拉低
#define GSM_DTR_ON			_IOW(GSM_IOC_MAGIC,7, int)	//DTR脚拉高
#define GSM_DTR_OFF			_IOW(GSM_IOC_MAGIC,8, int)	//DTR脚拉低
#define GSM_DCD_ON			_IOW(GSM_IOC_MAGIC,9, int)	//DCD脚拉高
#define GSM_DCD_OFF			_IOW(GSM_IOC_MAGIC,10, int)	//DCD脚拉低
#define GSM_CTS_ON			_IOW(GSM_IOC_MAGIC,11, int)	//CTS脚拉高
#define GSM_CTS_OFF			_IOW(GSM_IOC_MAGIC,12, int)	//CTS脚拉低
#define GSM_RTS_ON			_IOW(GSM_IOC_MAGIC,13, int)	//RTS脚拉高
#define GSM_RTS_OFF			_IOW(GSM_IOC_MAGIC,14, int)	//RTS脚拉低
#define GSM_GET_CTS			_IOR(GSM_IOC_MAGIC,15, int)	//得到CTS的值
#define GSM_GET_RTS			_IOR(GSM_IOC_MAGIC,16, int)	//得到RTS的值
#define GSM_GET_DCD			_IOR(GSM_IOC_MAGIC,17, int)	//得到DCD的值
#define GSM_IOC_MAXNR		17
...
static int gprs_ioctl (struct inode *inode , struct file *file, unsigned int cmd, unsigned long data)
{
		...
		switch(cmd)
		{
				...
				case GSM_PWR_OFF:
                        if(dbg)
                            pr_alert("GSM_PWR_OFF\n");
						set_gprspwr(false);
						break;
				case GSM_SOFTONOFF_ON:
                        if(dbg)
                            pr_alert("GSM_SOFTONOFF_ON\n");
						set_gprsonoff(true);
						break;
				case GSM_SOFTONOFF_OFF:
                        if(dbg)
                            pr_alert("GSM_SOFTONOFF_OFF\n");
						set_gprsonoff(false);
						break;
				case GSM_RST_ON:
                        if(dbg)
				...
				case GSM_RTS_OFF:
                        if(dbg)
                            pr_alert("GSM_RTS_OFF\n");
						set_gprsrts(false);
						break;
				case GSM_GET_CTS:
                        if(dbg)
                            pr_alert("GSM_GET_CTS\n");
						{
                ...
                case GSM_GET_DCD:
                        if(dbg)
                            pr_alert("GSM_GET_DCD\n");
                        {
							unsigned int status = (ioread8(gprs_dev.reg_gprs_cs1)>>4)&0x01;
        					ret = copy_to_user((unsigned int *)data,(unsigned int *)&status, sizeof(unsigned int));
                        }
                        break;
				default:
...
static int __gprs_init(void)
{
		...
		if(sand_machine()==SAND_IPS420)
		{
				...
				gpio_direction_output(IOMUX_TO_GPIO(IPS420_PIN_GPRS_PWR), 1);	
		}
		else if(sand_machine()==SAND_PS4KV2)
		{
