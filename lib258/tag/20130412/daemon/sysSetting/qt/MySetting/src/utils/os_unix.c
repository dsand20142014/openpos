...
int os_get_random(unsigned char *buf, size_t len)
{
	...
	if (f == NULL) {
        _print11("Could not open /dev/urandom.\n");
		return -1;
	}
...
void os_program_deinit(void)
{
	...
	dl_list_for_each(a, &alloc_list, struct os_alloc_trace, list) {
		...
		if (a->magic != ALLOC_MAGIC) {
            wpa_print11f(MSG_INFO, "MEMLEAK[%p]: invalid magic 0x%x "
				   "len %lu",
				   a, a->magic, (unsigned long) a->len);
			continue;
		}
        wpa_print11f(MSG_INFO, "MEMLEAK[%p]: len %lu",
			   a, (unsigned long) a->len);
		wpa_trace_dump("memleak", a);
	}
	if (total)
        wpa_print11f(MSG_INFO, "MEMLEAK: total %lu bytes",
			   (unsigned long) total);
#endif /* WPA_TRACE */
}
...
void * os_realloc(void *ptr, size_t size)
{
	...
	if (a->magic != ALLOC_MAGIC) {
        wpa_print11f(MSG_INFO, "REALLOC[%p]: invalid magic 0x%x%s",
			   a, a->magic,
...
void os_free(void *ptr)
{
	...
	a = (struct os_alloc_trace *) ptr - 1;
	if (a->magic != ALLOC_MAGIC) {
        wpa_print11f(MSG_INFO, "FREE[%p]: invalid magic 0x%x%s",
			   a, a->magic,
			   a->magic == FREED_MAGIC ? " (already freed)" : "");
