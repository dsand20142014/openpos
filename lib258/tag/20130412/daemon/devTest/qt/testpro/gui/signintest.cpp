...
void MyThread::run()
{
    ...
    while(runFlag){
        ...
        if (samp.pressure > 0) {
            //in sign zone
            if((samp.x>label_y)&&(samp.x<label_y+label_h) && (240-samp.y>label_x)&&(240-samp.y<label_w+label_x))
            {
                if(mode == 0x3)
                {
    ...
    BaseForm(tr("SIGN IN TEST"),parent)
{
    ...
    mythread =  new MyThread(xm,ym,240,211,this);
    mythread->start();
    _print("signin begin11111\n");

    btnOk->setText(tr("Print"));
    btnOk->show();
    btnOk->setFocus();
    btnlayout->addWidget(btnOk);
    _print("signin begin2222\n");

    clearBtn = new Button(tr("Clear"));
    btnlayout->addWidget(clearBtn);
    _print("signin begin3333\n");

    clearBtn->setMinimumSize(68, 36);
    btnOk->setMinimumSize(68, 36);
    btnExit->setMinimumSize(68, 36);

    _print("signin begin4444\n");
...
void SignInTest::clearButton_clicked()
{
   setbtnEnable(false);
    ...
    for(int i=0;i<240;i++)
        memset(mythread->fbuffer+50*2+i*640,0xFF,211*2);
