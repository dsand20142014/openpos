...
ApnTest::ApnTest(QWidget *parent) :
    BaseForm("GSM STATUS TEST",parent)
{
    flag = false;
    textEdit = new QTextEdit;
    layout->addWidget(textEdit);

    textEdit->setText("PRESS OK TO TEST!");
...
void ApnTest::apnConnect()
{
    ...
    if(!flag)
        ucResult =  OSGSM_GprsSetAPN((uchar*)"CMNET");
    else
       ucResult =  OSGSM_GprsSetAPN((uchar*)"MPOSCUPSH.SH");

     //qDebug()<<"000 ucResult"<<ucResult;
    if( !ucResult )
    {
      Os__xdelay(100);
      ucResult = OSGSM_GprsCheckDial(1200);
    }
    ...
    if(!ucResult){
       ...
       textEdit->append("APN : MPOSCUPSH.SH");
       textEdit->append("BAND : "+ QString(band));
       textEdit->append("REDIAL...");

       QTimer::singleShot(3000,this,SLOT(apnConnect()));

    }
    else{
        textEdit->append("TEST FAILED!");
        btnOk->setEnabled(true);
    }
