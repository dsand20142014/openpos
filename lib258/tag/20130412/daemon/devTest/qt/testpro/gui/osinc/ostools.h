...
/**
 * @fn int gb2312_utf8(char *sourcebuf,size_t sourcelen,char *destbuf,size_t destlen)
 * @brief	将GB2312格式字符串转换为UTF-8格式字符串
 *
 * @param sourcebuf	GB2312字符串地址
 * @param sourcelen	GB2312字符串长度
 * @param destbuf	转换后的UTF-8字符串保存地址
 * @param destlen	UTF-8的字符串最大长度
 *
 * @return 成功转换返回0,失败返回-1
 *
 */
int gb2312_utf8(char *sourcebuf,size_t sourcelen,char *destbuf,size_t destlen);

/**
 * @fn int utf8_gb2312(char *sourcebuf,size_t sourcelen,char *destbuf,size_t destlen)
 * @brief	将UTF-8格式字符串转换为GB2312格式字符串
 *
 * @param sourcebuf	UTF-8字符串地址
 * @param sourcelen	UTF-8字符串长度
 * @param destbuf	转换后的GB2312字符串保存地址
 * @param destlen	GB2312的字符串最大长度
 *
 * @return 成功转换返回0,失败返回-1
 *
 */
...
/**
 * @fn long DATE_ValideDateStr(char *pcDate)
 * @brief	检查日期格式是否正确
 *
 * @param pcDate 日期格式字符串,格式为YYYYMMDD
 *
 * @return 日期格式正确返回0,错误返回其他值
 *
 * @code
 * unsigned char *date = "20121012";
 * printf("the date is %s\n",date);
 * printf("the check result is %d\n",DATE_ValideDateStr(date));
 * //运行结果
 * the date is 20121012
 * the check result is 0
 * end
 * @endcode
 */
...
/**
 * @fn long DATE_ValideTimeStr(char *pcTime)
 * @brief	检查时间格式是否正确
 *
 * @param pcDate 时间格式字符串,格式为HHMMSS
 *
 * @return 时间格式正确返回0,错误返回其他值
 *
 * @code
 * unsigned char *time = "134820";
 * printf("the time is %s\n",time);
 * printf("the check result is %d\n",DATE_ValideTimeStr(time));
 * //运行结果
 * the time is 134820
 * the check result is 0
 * end
