...
//    printf("                qt g_app_cnt %d\n", g_app_cnt);
    ...
    for(ret = 0; ret < MAX_LINE; ret++)
    {
        ...
        sandlabel[ret]->setAlignment(Qt::AlignLeft);
        sandlabel[ret]->setAlignment(Qt::AlignVCenter);
        sandlabel[ret]->clear();
...
{    
 ...
 //       Os__light_on();
    ...
    {   //GPRS
        ...
        {
            char imagepath[20];
            memset(imagepath, 0, 20);

            top_form->label_gprs->clear();
            sprintf(imagepath,"signal%d.png", ret-1);//xl
            qt_display_image(0, 0, imagepath);
        }
		else if (ret >= 11 && ret <= 15)//2-5表示信号
        {
            ...
            qt_display_image(0, 0, "g.png");
            sprintf(imagepath,"signal%d.png", ret-11);//xl
            qt_display_image(0, 0, imagepath);
        }
    }
        break;
    ...
    case ID_CHRG_DETECT: // no use
        ...
        if (msg.msg_data.gprs_d)
            qt_display_image(1, 0, "connect.png");
        else
    ...
    {        
        ...
        if (ret>=0 && ret<=4)
        {
            sprintf(jpg_filename,"battery%d.png", ret);
            qt_display_image(1, 0, jpg_filename);
        }
        break;
    }
