/****************************************************************************
** Meta object code from reading C++ file 'qimeframe.h'
**
** Created: Tue Dec 4 10:36:24 2012
**      by: The Qt Meta Object Compiler version 63 (Qt 4.8.2)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../qSandIME/googlepinyin/qimeframe.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'qimeframe.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 63
#error "This file was generated using the moc from 4.8.2. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_IMFrame[] = {

 // content:
       6,       // revision
       0,       // classname
       0,    0, // classinfo
       5,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       1,       // signalCount

 // signals: signature, parameters, type, tag, flags
      11,    9,    8,    8, 0x05,

 // slots: signature, parameters, type, tag, flags
      35,    8,    8,    8, 0x0a,
      51,    9,    8,    8, 0x0a,
      71,    8,    8,    8, 0x08,
      92,    8,    8,    8, 0x08,

       0        // eod
};

static const char qt_meta_stringdata_IMFrame[] = {
    "IMFrame\0\0t\0imeTypeChanged(ImeType)\0"
    "changeImeType()\0setImeType(ImeType)\0"
    "sendContent(QString)\0confirmContent()\0"
};

void IMFrame::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Q_ASSERT(staticMetaObject.cast(_o));
        IMFrame *_t = static_cast<IMFrame *>(_o);
        switch (_id) {
        case 0: _t->imeTypeChanged((*reinterpret_cast< ImeType(*)>(_a[1]))); break;
        case 1: _t->changeImeType(); break;
        case 2: _t->setImeType((*reinterpret_cast< ImeType(*)>(_a[1]))); break;
        case 3: _t->sendContent((*reinterpret_cast< const QString(*)>(_a[1]))); break;
        case 4: _t->confirmContent(); break;
        default: ;
        }
    }
}

const QMetaObjectExtraData IMFrame::staticMetaObjectExtraData = {
    0,  qt_static_metacall 
};

const QMetaObject IMFrame::staticMetaObject = {
    { &QWSInputMethod::staticMetaObject, qt_meta_stringdata_IMFrame,
      qt_meta_data_IMFrame, &staticMetaObjectExtraData }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &IMFrame::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *IMFrame::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *IMFrame::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_IMFrame))
        return static_cast<void*>(const_cast< IMFrame*>(this));
    return QWSInputMethod::qt_metacast(_clname);
}

int IMFrame::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QWSInputMethod::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 5)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 5;
    }
    return _id;
}

// SIGNAL 0
void IMFrame::imeTypeChanged(ImeType _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 0, _a);
}
QT_END_MOC_NAMESPACE
