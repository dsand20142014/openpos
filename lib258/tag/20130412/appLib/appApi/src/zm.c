...
#ifndef DSZ
    //vfile("zsbhdr: %c %d %s %lx", Usevhdrs?'v':'f', len, frametypes[type+FTOFFSET], rclhdr(hdr));
#endif
    if (type == ZDATA)
        for (n = Znulls; --n >=0; )
            xsendline(0);
...
#ifndef DSZ
    //vfile("zshhdr: %c %d %s %lx", Usevhdrs?'v':'f', len,frametypes[type+FTOFFSET], rclhdr(hdr));
#endif
    //Uart_Printf("%s1\n\r", __func__);
    sendline(ZPAD);
    sendline(ZPAD);
    sendline(ZDLE);
    //Uart_Printf("%s2, Usevhdrs=%d\n\r", __func__, Usevhdrs);
    if (Usevhdrs)
    {
        sendline(ZVHEX);
        zputhex(len);
...
fifi:
    ...
    default:
        ...
        if (c >= -4 && c <= FRTYPES)
            Uart_Printf("zgethdr: %c %d %s %lx", Rxframeind, Rxhlen, frametypes[c+FTOFFSET], Rxpos);
        else
            Uart_Printf("zgethdr: %c %d %lx", Rxframeind, c, Rxpos);
