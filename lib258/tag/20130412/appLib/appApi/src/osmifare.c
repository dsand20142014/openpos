...
DRV_MMAP *myOSMFDrv;//ÓÃÓÚÖ±½Ó·ÃÎÊ
...
//        munmap(myOSMFDrv, 1024);
//        myOSMFDrv = (DRV_MMAP *)mmap(0, 1024, PROT_READ | PROT_WRITE, MAP_SHARED, fd_mifare, 0);
//
//        if (myOSMFDrv == MAP_FAILED) {
//                perror("OSDRV_Active mmap for PCD");
//                return -2;
//        }
...
//unsigned char OSMIFARE_Init(void)
//{
//        unsigned char ucStatus=0;
//
//        ucStatus=OSMIFARE_Open();
//
//#ifdef  M1_TEST
//
//        printf("@@@@@ %s \n", __func__);
...
DRVOUT *OSMIFARE_Polling( void )
{
        ...
        pOSMIFAREDrvOut->ucGenStatus=DRV_ENDED;
        return pOSMIFAREDrvOut;

        OSMIFAREDrvIn.ucType = OSMIFARE_CMD_POLLING;
        OSMIFAREDrvIn.ucLen = 0;
        OSMIFARE_DrvActive();
       // return((DRVOUT *)&OSMIFAREDrvOut);
