...
ETC_HANDLE  FindAllSectionInEtc (ETC_HANDLE hEtc, char section[][50], int sectionSize);
/*****************	系统参数的函数    ****************************************/
int PARAM_setting_get(SYS_SETTING *setting)
{
	int ret = -1, cn = 0, i;
	int etcFile,etcSection;
	unsigned char ucParm[200];
	char *pstr1 = NULL, *pstr2 = NULL;
	memset(ucParm,0,sizeof(ucParm));
	sprintf(ucParm,"%s%s",SPD_APPMANAGER_PATH,SPD_FILE_DAEMONSET);
	etcFile = Os__param_loadFullPath(ucParm);
	if(etcFile > 0)
	{
  		etcSection = Os__param_findSection(etcFile,setting->section,0);
  		if(etcSection > 0)
  		{
  			//读取value值,包涵type brief,briefList
  			memset(ucParm,0,sizeof(ucParm));
  			Os__param_get(etcSection,setting->key,ucParm,sizeof(ucParm));
  			//查找#字符位置
  			pstr1 = ucParm;
  			pstr2 = strstr(pstr1,"#");
  			if(pstr2 != NULL)
  			{
  				//获取value值,最大获取49个字符
  				for(i=0;i<pstr2-pstr1;i++)
  					if(*(pstr1+i) != ' ' && *(pstr1+i) != '\t' && cn < sizeof(setting->value)-1)
  						setting->value[cn++] = *(pstr1+i);
  				cn = 0;
  				//获取#号后type值
  				setting->type = *(pstr2+1) - '0';
  				//获取|后的字符串
  				pstr1 = strstr(pstr2,"|");
  				if(pstr1 != NULL)
  				{
					...
					pstr2 = strtok(pstr1," ");
					do{
						if(pstr2 != NULL && cn < BRIEF_LIST_MAX)
						{
							while(*pstr2 != '\0')
							{
								if(*pstr2++ == '-')
									strncpy(setting->briefList[cn++],pstr2,49);
							}
						}
						else
							break;
					}while((pstr2=strtok(NULL," ")));
  				}
  			}
  			else
  				strncpy(setting->value,ucParm,sizeof(setting->value));
  			ret = cn;
  		}
...
int PARAM_setting_set(SYS_SETTING *setting)
{
	...
	if(etcFile > 0)
	{
  		...
  		if(etcSection > 0)
  		{
  			...
  			if(pstr2 != NULL)
  			{
  				sprintf(ucTmpBuf,"%s %s",setting->value,pstr2);
  			}
  			else
  				sprintf(ucTmpBuf,"%s",setting->value);
  				
  			Os__param_set(etcSection,setting->key,ucTmpBuf);
  			memset(ucParm,0,sizeof(ucParm));
			sprintf(ucParm,"%s%s",SPD_APPMANAGER_PATH,SPD_FILE_DAEMONSET);
  			Os__param_saveFullPath(etcFile,ucParm);
  			ret = 0;
  		}
  		Os__param_unload(etcFile);
  	}
...
int PARAM_shell_get(PROCESS_INIT_INFO proInitList[], int proMaxNum, int proMode)
{
	...
	memset(ucParm,0,sizeof(ucParm));
	if(proMode == DRIVERS_SHELL)
		sprintf(ucParm,"%s%s",SPD_DRIVER_PATH,SPD_FILE_DRVSH);
	else
		sprintf(ucParm,"%s%s",SPD_APPMANAGER_PATH,SPD_FILE_DAEMONSH);
	fd = fopen(ucParm,"r");
	...
	if(proMode == DRIVERS_SHELL)
	{
		while(fgets(ucParm,sizeof(ucParm),fd))
		{
			pstr1 = strstr(ucParm,"insmod");
			pstr2 = strstr(ucParm,"dbg=");
			if(pstr1 != NULL && pstr2 != NULL && i<proMaxNum)
			{
				if(ucParm[0] == '#')
					proInitList[i].insmod = 0;
				else
					proInitList[i].insmod = 1;
				...
				memset(proInitList[i].name,0,sizeof(proInitList[i].name));
				
				pstr1 = strstr(ucParm,SPD_DRIVER_PATH);
				pstr2 = strstr(pstr1,".ko.");
				...
				if(pstr2 != NULL)
				{
					strncpy(proInitList[i].name,pstr1+strlen(SPD_DRIVER_PATH),pstr2-pstr1-strlen(SPD_DRIVER_PATH)+3);
					pstr1 = strstr(pstr2," ");
					strncpy(proInitList[i].version,pstr2+4,pstr1-pstr2-4);
				}
				else
				{
					//查找不到版本号时
					pstr2 = strstr(pstr1," ");
					strncpy(proInitList[i].name,pstr1+strlen(SPD_DRIVER_PATH),pstr2-pstr1-strlen(SPD_DRIVER_PATH));
					//strcpy(proInitList[i].version,"0");
				}
				i++;
			}
			memset(ucParm,0,sizeof(ucParm));
	...
	else
	{
		while(fgets(ucParm,sizeof(ucParm),fd))
		{
			pstr1 = strstr(ucParm,SPD_APPMANAGER_PATH);
			pstr2 = strstr(ucParm,"dbg=");
			if(pstr1 != NULL && i<proMaxNum)
			{
				if(ucParm[0] == '#')
					proInitList[i].insmod = 0;
				else
				...
				pstr2 = strstr(pstr1,".dm.");
				if(pstr2 != NULL)
				{
					strncpy(proInitList[i].name,pstr1+strlen(SPD_APPMANAGER_PATH),pstr2-pstr1-strlen(SPD_APPMANAGER_PATH)+3);
					pstr1 = strstr(pstr2," ");
					strncpy(proInitList[i].version,pstr2+4,pstr1-pstr2-4);
				}
				else
				{
					//查找不到版本号时
					pstr2 = strstr(pstr1," ");
					strncpy(proInitList[i].name,pstr1+strlen(SPD_APPMANAGER_PATH),pstr2-pstr1-strlen(SPD_APPMANAGER_PATH));
					//strcpy(proInitList[i].version," ");
				}
				i++;
			}
			memset(ucParm,0,sizeof(ucParm));
...
int PARAM_shell_set(PROCESS_INIT_INFO proInitList[], int tolProNum, int proMode)
{
	...
	if(fd == NULL)
		return -1;
	memset(ucParm,0,sizeof(ucParm));
	sprintf(ucParm,"#!/bin/sh\n");
	fwrite(ucParm,1,strlen(ucParm),fd);
	system("sync");
	
	for(i=0;i<tolProNum;i++)
	{
		memset(ucParm,0,sizeof(ucParm));
		
		if(proInitList[i].insmod == 0)
			ucParm[0] = '#';
		else
			ucParm[0] = ' ';
		...
		if(proMode == DRIVERS_SHELL)
		{
			sprintf(ucParm,"%sinsmod %s%s",ucParm,SPD_DRIVER_PATH,proInitList[i].name);
			if(proInitList[i].version[0] != '\0')
				sprintf(ucParm,"%s.%s",ucParm,proInitList[i].version);
			sprintf(ucParm,"%s dbg=%d\n",ucParm,proInitList[i].dbg);
		}
		else
		{
			sprintf(ucParm,"%s%s%s",ucParm,SPD_APPMANAGER_PATH,proInitList[i].name);
			
			if(proInitList[i].version[0] != '\0')
				sprintf(ucParm,"%s.%s",ucParm,proInitList[i].version);
			
			if(strstr(proInitList[i].name,"qLcdDaemon.dm") != NULL)
				sprintf(ucParm,"%s -qws",ucParm);
		
			
			if(proInitList[i].dbg == 0)
				sprintf(ucParm,"%s dbg=0 &\n",ucParm);
			else
				sprintf(ucParm,"%s dbg=1 &\n",ucParm);
		}
		fwrite(ucParm,1,strlen(ucParm),fd);
	}
...
{
	return FindAllSectionInEtc(hEtc, section, sectionSize);
}
...
ETC_HANDLE  FindAllSectionInEtc (ETC_HANDLE hEtc, char section[][50], int sectionSize)
{
	...
	for (i = 0; i < petc->section_nr; i++)
	{
		if (petc->sections [i].name != NULL && i < sectionSize)
		{
			strncpy(section[i],petc->sections [i].name,sizeof(section[i]));
		}
		else
			return i;
	}
