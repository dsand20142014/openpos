  ...
  if(wrq.u.data.length >= 300)
    {
      ...
      Uart_Printf("%-8.16s  Recommend Wireless Extension v%d or later,\n",
	     ifname, range->we_version_source);
      Uart_Printf("          Currently compiled with Wireless Extension v%d.\n\n",
...
iw_print_version_info(const char *	toolname)
{
  ...
  if(toolname != NULL)
    Uart_Printf("%-8.16s  Wireless-Tools version %d\n", toolname, WT_VERSION);
  Uart_Printf("          Compatible with Wireless Extension v11 to v%d.\n\n",
	 WE_MAX_VERSION);
  ...
  if(we_kernel_version > 15)
    Uart_Printf("Kernel    Currently compiled with Wireless Extension v%d.\n\n",
	   we_kernel_version);
	   ...
	   struct sockaddr *sap)
{
  ...
  if(strchr(bufp, ':') == NULL)
	 ...
	 !(arp_query.arp_flags & ATF_COM))
	{
      ...
      {
	char buf[20];
	Uart_Printf("IP Address %s => Hw Address = %s\n",
...
#ifdef DEBUG
  Uart_Printf("DBG - stream->current = %p, stream->value = %p, stream->end = %p\n",
...
#ifdef DEBUG
  Uart_Printf("DBG - iwe->cmd = 0x%X, iwe->len = %d\n",
	 iwe->cmd, iwe->len);
#endif
...
#ifdef DEBUG
  Uart_Printf("DBG - event_type = %d, event_len = %d, pointer = %p\n",
	  ...
	  if(descr == NULL)
	      ...
	      if((token_len != extra_len) && (extra_len >= 4))
		{
		  ...
		  if((alt_token_len + 8) == extra_len)
		    {
#ifdef DEBUG
		      Uart_Printf("DBG - alt_token_len = %d\n", alt_token_len);
#endif
		...
		iwe->u.data.pointer = NULL;	/* Discard paylod */
#ifdef DEBUG
	      Uart_Printf("DBG - extra_len = %d, token_len = %d, token = %d, max = %d, min = %d\n",
  ...
  if(wrq.u.data.length)
    {
      ...
      int	i;
      Uart_Printf("Scan result [%02X", buffer[0]);
