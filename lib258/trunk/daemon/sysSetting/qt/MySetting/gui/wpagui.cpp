...
//    connectButton->setObjectName("wifibtn");
//    disconnectButton->setObjectName("wifibtn");
//    scanButton->setObjectName("wifibtn");

    connectButton->setStyleSheet("QPushButton{font:14px;}");
    disconnectButton->setStyleSheet("QPushButton{font:14px;}");
    scanButton->setStyleSheet("QPushButton{font:14px;}");
	...
	if (openCtrlConnection(ctrl_iface) < 0) {
        _print("Failed to open control connection to wpa_supplicant.\n");
	}
...
int WpaGui::openCtrlConnection(const char *ifname)
{
	...
	if (wpa_ctrl_attach(monitor_conn)) {
        _print("Failed to attach to wpa_supplicant\n");
		wpa_ctrl_close(monitor_conn);
		monitor_conn = NULL;
		wpa_ctrl_close(ctrl_conn);
		ctrl_conn = NULL;
...
//    //qDebug()<<"ret  wpa_ctrl_request ==="<<ret;
	if (ret == -2)
        _print("'%s' command timed out.\n", cmd);
	else if (ret < 0)
        _print("'%s' command failed.\n", cmd);
...
void WpaGui::updateStatus()
{
	...
	if (ctrl_conn == NULL || ctrlRequest("STATUS", buf, &len) < 0) {
        textStatus->setText(tr("Could not get status from wpa_supplicant"));
...
void WpaGui::ping()
{
	...
	if (ctrlRequest("PING", buf, &len) < 0) {
        _print("PING failed - trying to reconnect\n");
		if (openCtrlConnection(ctrl_iface) >= 0) {
...
int WpaGui::getNetworkDisabled(const QString &sel)
{
	...
	int pos = cmd.indexOf(':');
	if (pos < 0) {
        _print("Invalid getNetworkDisabled '%s'\n",cmd.toAscii().constData());
		return -1;
	}
	cmd.truncate(pos);
...
void WpaGui::selectAdapter( const QString & sel )
{
	if (openCtrlConnection(sel.toAscii().constData()) < 0)
        _print("Failed to open control connection to "
		       "wpa_supplicant.\n");
	updateStatus();
	updateNetworks();
}
...
void WpaGui::createTrayIcon(bool trayOnly)
{
	...
	if (QImageReader::supportedImageFormats().contains(QByteArray("svg")))
        tray_icon->setIcon(QIcon("./pic/wpa_gui.svg"));
	else
        tray_icon->setIcon(QIcon("./pic/wpa_gui.png"));
...
bool WpaGui::serviceRunning()
{
	...
	scm = OpenSCManager(0, 0, SC_MANAGER_CONNECT);
	if (!scm) {
        _print("OpenSCManager failed: %d\n", (int) GetLastError());
		return false;
	}

	svc = OpenService(scm, WPASVC_NAME, SERVICE_QUERY_STATUS);
	if (!svc) {
        _print("OpenService failed: %d\n\n", (int) GetLastError());
		CloseServiceHandle(scm);
		return false;
	}
...
void WpaGui::keyPressEvent(QKeyEvent *e)
{
    switch(e->key())
    {
        case Qt::Key_Escape:
