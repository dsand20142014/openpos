...
QString NetworkConfig::psw_temp = "";
...
void NetworkConfig::addNetwork()
{
    ...
    if(new_network)
        psw_temp = pskEdit->text();

    //qDebug()<<"psw_temp"<<psw_temp.toAscii().data();
    ...
    //printf("new_network=====%d \n",new_network);
   // if(!new_network){
        WifiNetwork wifi;
        memset(&wifi,0,sizeof(WifiNetwork));
        //printf("ssidEdit->text().toAscii().data()====%s \n",ssidEdit->text().toAscii().data());

        //printf("psw_temp.toAscii().data()====%s \n",psw_temp.toAscii().data());
        sprintf(wifi.ssid,ssidEdit->text().toAscii().data());
        sprintf(wifi.psk,psw_temp.toAscii().data());
        int rec =  Os__setWifi(wifi);
        //printf("~~~~~~~~~~~~~Os__setWifi ===%d \n",rec);
        ...
        strcpy(sysSetting.key,"AP_NAME");
        strncpy(sysSetting.value,wifi.ssid,sizeof(sysSetting.value));
        PARAM_setting_set(&sysSetting);

        memset(&sysSetting,0,sizeof(SYS_SETTING));
        strcpy(sysSetting.section,"WIFI_SETTING");
        strcpy(sysSetting.key,"PASSWD");
        strncpy(sysSetting.value,wifi.psk,sizeof(sysSetting.value));
        PARAM_setting_set(&sysSetting);
        //add end
  //  }
