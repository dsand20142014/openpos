	...
	: QDialog(parent)
{
	...
	{
        default_icon = new QIcon("./pic/wpa_gui.svg");
        ap_icon = new QIcon("./pic/ap.svg");
        laptop_icon = new QIcon("./pic/laptop.svg");
        group_icon = new QIcon("./pic/group.svg");
        invitation_icon = new QIcon("./pic/invitation.svg");
	} else {
        default_icon = new QIcon("./pic/wpa_gui.png");
        ap_icon = new QIcon("./pic/ap.png");
        laptop_icon = new QIcon("./pic/laptop.png");
        group_icon = new QIcon("./pic/group.png");
        invitation_icon = new QIcon("./pic/invitation.png");
	}
...
void Peers::ctx_p2p_req_pin()
{
	...
	if (wpagui->ctrlRequest(cmd, reply, &reply_len) < 0) {
		QMessageBox msg;
		msg.setIcon(QMessageBox::Warning);
        msg.setText(tr("Failed to request\n PIN from peer."));
		msg.exec();
	}
}
...
void Peers::ctx_p2p_show_pin()
{
	...
	if (wpagui->ctrlRequest(cmd, reply, &reply_len) < 0) {
		QMessageBox msg;
		msg.setIcon(QMessageBox::Warning);
        msg.setText(tr("Failed to request peer\n to enter PIN."));
		msg.exec();
	}
}
...
void Peers::ctx_p2p_display_pin_pd()
{
	...
	if (wpagui->ctrlRequest(cmd, reply, &reply_len) < 0) {
		QMessageBox msg;
		msg.setIcon(QMessageBox::Warning);
        msg.setText("Failed to initiate P2P\n connect.");
		msg.exec();
		return;
	}
	reply[reply_len] = '\0';
...
void Peers::learn_ap_config()
{
	...
	if (wpagui->ctrlRequest(cmd, reply, &reply_len) < 0) {
		QMessageBox msg;
		msg.setIcon(QMessageBox::Warning);
        msg.setText(tr("Failed to start learning\n AP configuration."));
		msg.exec();
	}
}
...
void Peers::ctx_p2p_start_persistent()
{
	...
	if (wpagui->ctrlRequest(cmd, reply, &reply_len) < 0 ||
		...
		QMessageBox msg;
		msg.setIcon(QMessageBox::Warning);
        msg.setText(tr("Failed to start persistent\n P2P Group."));
		msg.exec();
...
void Peers::ctx_p2p_invite()
{
	...
	if (wpagui->ctrlRequest(cmd, reply, &reply_len) < 0 ||
		...
		QMessageBox msg;
		msg.setIcon(QMessageBox::Warning);
        msg.setText(tr("Failed to invite peer\n to start persistent "
			       "P2P Group."));
		msg.exec();
	}
}
...
void Peers::on_closeButton_clicked()
{
    close();
}
