...
try {
    ...
    for (i=0; i<n; i++) {
        ...
        connect(checkBox, SIGNAL(clicked()), this, SLOT(chb_check()));
...
{
    QCheckBox *checkBoxAuto, *checkBoxChb;
    for(int i=0; i<tableWidget->rowCount(); i++)
    {
        ...
        if(checkBoxAuto->isChecked())
        {
            ...
            checkBoxChb = (QCheckBox *)tableWidget->cellWidget(i, COL_CHB);
            if(checkBoxChb->isChecked())
                checkBoxChb->setChecked(false);
        }
    }
}

void AppManagement::chb_check()
{
    QCheckBox *checkBoxAuto, *checkBoxChb;
    for(int i=0; i<tableWidget->rowCount(); i++)
    {
        ...
        {
            checkBoxAuto = (QCheckBox *)tableWidget->cellWidget(i, COL_AUTO);
            if(checkBoxAuto->isChecked() && checkBoxAuto == actionAuto)
            {
