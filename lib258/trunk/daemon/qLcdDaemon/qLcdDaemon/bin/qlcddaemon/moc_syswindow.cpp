/****************************************************************************
** Meta object code from reading C++ file 'syswindow.h'
**
** Created: Tue Dec 4 10:36:47 2012
**      by: The Qt Meta Object Compiler version 63 (Qt 4.8.2)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../qlcddaemon/syswindow.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'syswindow.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 63
#error "This file was generated using the moc from 4.8.2. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_SysWindow[] = {

 // content:
       6,       // revision
       0,       // classname
       0,    0, // classinfo
       6,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: signature, parameters, type, tag, flags
      11,   10,   10,   10, 0x08,
      41,   10,   10,   10, 0x08,
      71,   10,   10,   10, 0x08,
     103,   10,   10,   10, 0x08,
     132,   10,   10,   10, 0x08,
     165,  163,   10,   10, 0x08,

       0        // eod
};

static const char qt_meta_stringdata_SysWindow[] = {
    "SysWindow\0\0on_pushButton_enter_clicked()\0"
    "on_pushButton_reset_clicked()\0"
    "on_pushButton_setting_clicked()\0"
    "on_pushButton_test_clicked()\0"
    "on_pushButton_cancel_clicked()\0e\0"
    "keyPressEvent(QKeyEvent*)\0"
};

void SysWindow::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Q_ASSERT(staticMetaObject.cast(_o));
        SysWindow *_t = static_cast<SysWindow *>(_o);
        switch (_id) {
        case 0: _t->on_pushButton_enter_clicked(); break;
        case 1: _t->on_pushButton_reset_clicked(); break;
        case 2: _t->on_pushButton_setting_clicked(); break;
        case 3: _t->on_pushButton_test_clicked(); break;
        case 4: _t->on_pushButton_cancel_clicked(); break;
        case 5: _t->keyPressEvent((*reinterpret_cast< QKeyEvent*(*)>(_a[1]))); break;
        default: ;
        }
    }
}

const QMetaObjectExtraData SysWindow::staticMetaObjectExtraData = {
    0,  qt_static_metacall 
};

const QMetaObject SysWindow::staticMetaObject = {
    { &QMainWindow::staticMetaObject, qt_meta_stringdata_SysWindow,
      qt_meta_data_SysWindow, &staticMetaObjectExtraData }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &SysWindow::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *SysWindow::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *SysWindow::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_SysWindow))
        return static_cast<void*>(const_cast< SysWindow*>(this));
    return QMainWindow::qt_metacast(_clname);
}

int SysWindow::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QMainWindow::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 6)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 6;
    }
    return _id;
}
QT_END_MOC_NAMESPACE
