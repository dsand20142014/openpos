...
void IMFrame::sendComString(const QString & str)
{
    ...
    if(pinyinDialog->isVisible())
    {
        pinyinDialog->ui->lineEdit->clear();
        pinyinDialog->cadidateStr.clear();
        pinyinDialog->hide();
    }
}
...
bool IMFrame::filter(int unicode, int keycode, int modifiers, bool isPress, bool autoRepeat)
{
    ...
    else
    {
        ...
        {
        case NUM:
            if(keycode==Qt::Key_F3||keycode==Qt::Key_F4||keycode==Qt::Key_Enter)
        ...
        case ABC:
            if(keycode == Qt::Key_F9||keycode==Qt::Key_F3||keycode==Qt::Key_F4||keycode==Qt::Key_Enter)
                return inputPinyin(unicode,keycode,modifiers,isPress,autoRepeat);
            else{
