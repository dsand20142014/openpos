...
class DictTrie : AtomDictBase {
 ...
 public:
  ...
  LemmaIdType put_lemma(char16 lemma_str[], uint16 splids[],
                        uint16 lemma_len, uint16 count) {(void)lemma_str;(void)splids;(void)lemma_len;(void)count; return 0;}

  LemmaIdType update_lemma(LemmaIdType lemma_id, int16 delta_count,
                           bool selected) {(void)lemma_id;(void)delta_count;(void)selected; return 0;}

  LemmaIdType get_lemma_id(char16 lemma_str[], uint16 splids[],
                           uint16 lemma_len) {(void)lemma_str;(void)splids;(void)lemma_len;return 0;}

  LmaScoreType get_lemma_score(LemmaIdType lemma_id) {(void)lemma_id;return 0;}

  LmaScoreType get_lemma_score(char16 lemma_str[], uint16 splids[],
                        uint16 lemma_len) {(void)lemma_str;(void)splids;(void)lemma_len;return 0;}

  bool remove_lemma(LemmaIdType lemma_id) {(void)lemma_id;return false;}
