...
TfTest::TfTest(int flag,QWidget *parent) : BaseForm(tr(" "), parent)
{
    ...
    flags = flag;
    if(flags==1)
        title->setText(tr("USB TEST"));
    else
        title->setText(tr("TF TEST"));

    textEdit->setText(tr("PRESS OK TO START TEST!"));
...
void TfTest::getResult()
{
    ...
    if(flags ==1)
        ...
        if(flags==1){
            if(ret==0)
                textEdit->setText(tr("USB MOUNT SUCCESS!"));
            else{
                textEdit->setText(tr("USB MOUNT FAILED,PRESS OK TO RETRY!"));//  usb mount failed! pess ok to retry");
        ...
        {
            if(ret==0)
                textEdit->setText(tr("TF MOUNT SUCCESS!"));
            else{
                textEdit->setText(tr("TF MOUNT FAILED, PRESS OK TO RETRY!"));//tf mount failed! pess ok to retry");
                btnOk->setEnabled(true);
        ...
        _print("write [%d]\n",ret);
        if(ret==0)
            textEdit->append(tr("WRITE FILE SUCCESS!"));
        else
            textEdit->append(tr("WRITE FILE FAILED!"));
        ...
        _print("append %d\n",ret);
        if(ret==0)
            textEdit->append(tr("APPEND  ! SUCCESS"));
        else
            textEdit->append(tr("APPEND ! FAILED!"));
        ...
        if(ret==0)
            textEdit->append(tr("CLOSE  FILE SUCCESS!"));
        else
            textEdit->append(tr("CLOSE FILE FAILED!"));
