...
Pro_PreAuth::Pro_PreAuth(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::Pro_PreAuth)
{
    ui->setupUi(this);
...
Pro_PreAuth::~Pro_PreAuth()
{
    delete ui;
}

void Pro_PreAuth::sysEventSlot(int sysEvent)
{
...
void Pro_PreAuth::destroyWindow(int)
{
//    a->terminate();
...
bool Pro_PreAuth::eventFilter(QObject *target, QEvent *event)
{
     QString str;
...
void Pro_PreAuth::slottimer1GetCardInfo()
{
...
void Pro_PreAuth::on_confirmButton_clicked()
{
...
//    desk->showFullScreen();
    ...
    //交易类型
    ProUiFace.UiToPro.ucTransType = TRANS_PREAUTH;
    ProUiFace.UiToPro.CardInfo.ucInputMode= TRANS_INPUTMODE_SWIPECARD;
    // 启动线程 处理交易
...
void Pro_PreAuth::on_cancelButton_clicked()
{
    disconnect(mainEntryThread,SIGNAL(sysEventSignal(int)),this,SLOT(sysEventSlot(int)));
    mainEntryThread->wakeUp();
    sandtimer->stop();
