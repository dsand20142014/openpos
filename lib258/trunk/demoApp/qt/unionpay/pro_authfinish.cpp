...
Pro_AuthFinish::Pro_AuthFinish(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::Pro_AuthFinish)
{
    ui->setupUi(this);
    ...
    ui->line2_authnum->installEventFilter(this);
...
Pro_AuthFinish::~Pro_AuthFinish()
{
    delete ui;
}
void Pro_AuthFinish::destroyWindow(int)
{
//    a->terminate();
...
void Pro_AuthFinish::sysEventSlot(int sysEvent)
{
...
bool Pro_AuthFinish::eventFilter(QObject *target, QEvent *event)
{
     QString str;
...
void Pro_AuthFinish::slottimer1GetCardInfo()
{
...
void Pro_AuthFinish::on_cancelButton_clicked()
{
    disconnect(mainEntryThread,SIGNAL(sysEventSignal(int)),this,SLOT(sysEventSlot(int)));
    mainEntryThread->wakeUp();
    sandtimer->stop();
...
void Pro_AuthFinish::on_confirmButton_clicked()
{
    int uiLen=0;
...
//    desk->showFullScreen();
    ...
    //交易类型
    ProUiFace.UiToPro.ucTransType = TRANS_PREAUTHFINISH;
    ProUiFace.UiToPro.CardInfo.ucInputMode= TRANS_INPUTMODE_SWIPECARD;
    // 启动线程 处理交易
