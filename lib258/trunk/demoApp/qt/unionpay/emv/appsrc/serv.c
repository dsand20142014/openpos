...
unsigned char SERV_Settle(unsigned char ucFlag)
{
	...
	memcpy(ProUiFace.ProToUi.aucLine1,(uchar *)"批次不符请签到", strlen((char *)"批次不符请签到") );
	    sem_post(&binSem1);
    sem_wait(&binSem2);
#else
		//Os__clr_display(255);
		//Os__GB2312_display(0,0,(unsigned char *)"批次不符请签到");
		MSG_WaitKey(3);
#endif		
		return(SUCCESS);
	}
	...
	memset((uchar*)&ProUiFace.ProToUi,0,sizeof(ProUiFace.ProToUi));
	ProUiFace.ProToUi.uiLines=1;	
	memcpy(ProUiFace.ProToUi.aucLine1,(uchar *)"密钥错误请签到", strlen((char *)"密钥错误请签到") );
	    sem_post(&binSem1);
    sem_wait(&binSem2);
#else
	    	//Os__clr_display(255);
		//Os__GB2312_display(0,0,(uchar *)"密钥错误请签到");
		MSG_WaitKey(3);
#endif		
		return(SUCCESS);
	}
...
unsigned char SERV_ProcessData(void)
{
	...
	{
		//Os__clr_display(255);
		//Os__GB2312_display(1,0,(unsigned char*)" 数据采集成功!");
		UTIL_GetKey(5);
	}
	else if (ucResult !=ERR_HOSTCODE)
	{
		//Os__clr_display(255);
		//Os__GB2312_display(1,0,(unsigned char*)" 数据采集失败");
		//Os__GB2312_display(2,1,(unsigned char*)" 请手工上送");
		UTIL_GetKey(5);
	}
	if (ucResult==ERR_HOSTCODE)
	{
		...
		if (G_RUNDATA_ucErrorExtCode==5)
			//Os__GB2312_display(1,1,(unsigned char*)" 数据格式错");
        //else if (G_RUNDATA_ucErrorExtCode==3)
			//Os__GB2312_display(1,1,(unsigned char*)"后台数据库错");
        //else
        //{
			//Os__GB2312_display(1,1,(unsigned char*)"数据采集失败");
			//Os__GB2312_display(2,1,(unsigned char*)" 请手工上送");
        //}
...
unsigned char SERV_SendCollectData(void)
{
	...
	switch(ucPress)
	{
		case KEY_ENTER:
			...
			G_NORMALTRANS_euCardSpecies=0xFF;				//用于区分使用哪种ISO属性结构
			//Os__clr_display(255);
	...
	if(ucResult == SUCCESS)
	{
		...
		if (DataSave0.ConstantParamData.ucCollectFlag=='1')
		{
			...
			for(uiI=0;uiI<uiTransTotal;uiI++)
			{
				...
				)
				{
					...
					util_Printf("DataSave0.ChangeParamData.uiSendTotalNb总笔数=%d\n",DataSave0.ChangeParamData.uiSendTotalNb);
		...
		if(ucResult==SUCCESS)
		{
			if(DataSave0.ConstantParamData.ucSelfLogoff == '1')
			{
				/*置签退标志*/
				//Os__clr_display(255);
				//Os__GB2312_display(1,0,(unsigned char *)"    结算完成");
				//Os__GB2312_display(2,0,(unsigned char *)"    正在签退...");
				MSG_WaitKey(3);
			...
			{	/*置签到标志*/
				//Os__clr_display(255);
				//Os__GB2312_display(1,0,(unsigned char *)"    结算完成");
				//Os__GB2312_display(2,0,(unsigned char *)"    正在签到...");
				MSG_WaitKey(3);
...
unsigned char SERV_SettleOnline(void)
{
	...
	//Os__clr_display(255);
	//Os__GB2312_display(1,0,(unsigned char*)"   正在批结算");
...
//		DataSave0.ConstantParamData.ucInternalSettleFlag=SETTLE_FAIL;
	...
	if (ucResult == SUCCESS)
	{
		...
		else
		{
			...
			//上送磁卡离线交易(实际包括IC/MAG的离线调整、离线结算、退货、预授权完成(离线))
...
unsigned char SERV_SettleBatchSend(unsigned int ucSentTransNum)
{
	...
	*/
	...
	do{
		//Os__clr_display(255);
		//Os__GB2312_display(1,0,(unsigned char *)"    结算不平");
		//Os__GB2312_display(2,0,(unsigned char *)"   正在批结算");
		//Os__GB2312_display(3,0,(unsigned char *)"     请稍候");
		UTIL_WaitGetKey(2);
		ISO8583_Clear();
...
unsigned char SERV_SettleOfflineSend(unsigned short uiSendFlag,unsigned int *pucOfflineNum)
{
	...
	//Os__clr_display(255);
	//Os__GB2312_display(1,0,(unsigned char*)"正在上送离线交易");
	//Os__GB2312_display(3,0,(unsigned char*)"     请稍候");
	util_Printf("\n------------------正在上送磁条卡离线交易-------1----------------\n");
	ucResult = SUCCESS;
	ucOfflineNum=0;
...
//		if(uiI++>=DataSave0.ChangeParamData.uiEMVICLogNum) return EMV_NOSUCHTRACE;
			...
			)
		{
			...
			ISO8583_SetBitValue(55, G_NORMALTRANS_ucField55Data, G_NORMALTRANS_uiField55Len);
                     ...
                     memset(aucBuf,0,sizeof(aucBuf));
			memcpy(aucBuf,"00",2);
#if 0               //此处在北京检测中心有如下要求,但在银联上海分公司测试不要求作如下处理
			switch(G_NORMALTRANS_ucTransType)
			{
				case TRANS_PREAUTH:
					memcpy(aucBuf,"10",2);
...
unsigned char SERV_ICInformTransSend(unsigned char ucSettleFlag,unsigned int *pucTransNum)
{
	...
	for(uiTransIndex=0;uiTransIndex<=uiTotalNb;uiTransIndex++)
       {
       //	//Os__clr_display(255);
	//	//Os__GB2312_display(0,0,(unsigned char*)"上送IC卡通知交易");
		GetOldTransData(uiTransIndex);
		OSMEM_Memcpy((unsigned char *)&(DataSave1.SaveTrans),
						(unsigned char *)&NormalTransData,
						sizeof(NORMALTRANS));
...
unsigned char SERV_SettleBatchEnd2(unsigned int ucSentTransNum)
{
	...
	*/
	...
	//Os__clr_display(255);
	//Os__GB2312_display(1,0,(unsigned char *)"    结算不平");
	//Os__GB2312_display(2,0,(unsigned char *)"   正在发送结束");
	//Os__GB2312_display(3,0,(unsigned char *)"     请稍候");
...
unsigned char SERV_ReprintSettle(void)
{
	...
	//Os__clr_display(255);
	//Os__GB2312_display(1,0,(uchar*)"按[确认]开始打印");
	//Os__GB2312_display(2,0,(uchar*)"     请确认");
...
//				//Os__GB2312_display(2, 0, (uchar * )" (qPBOC消费)");
				memcpy(aucbuf, (uchar * )" (qPBOC消费)",strlen((char*)" (qPBOC消费)"));

			else if(G_NORMALTRANS_ucECTrans ==TRANS_EC_PURCHASE)
//				//Os__GB2312_display(2, 0, (uchar * )" (EC消费)");
				memcpy(aucbuf, (uchar * )" (EC消费)",strlen((char*)" (EC消费)"));
			else
				////Os__GB2312_display(2, 0, (uchar * )" (消费)");
				memcpy(aucbuf, (uchar * )" (消费)",strlen((char*)" (消费)"));
			break;
		case TRANS_CUPMOBILE:
			////Os__GB2312_display(2, 0, (uchar * )" (移动支付消费)");
				memcpy(aucbuf, (uchar * )" (移动支付消费)",strlen((char*)" (移动支付消费)"));
   			break;
		case TRANS_VOIDPURCHASE:
			////Os__GB2312_display(2, 0, (uchar * )" (消费撤销)");
				memcpy(aucbuf, (uchar * )" (消费撤销)",strlen((char*)" (消费撤销)"));
			break;
		case TRANS_VOIDCUPMOBILE:
			////Os__GB2312_display(2, 0, (uchar * )" (移动支付撤销)");
				memcpy(aucbuf, (uchar * )" (移动支付撤销)",strlen((char*)" (移动支付撤销)"));
   			break;
		case TRANS_UNDOOFF:
			////Os__GB2312_display(2, 0, (uchar * )" (离线撤销)");
				memcpy(aucbuf, (uchar * )" (离线撤销)",strlen((char*)" (离线撤销)"));
			break;
		case TRANS_PREAUTH:
			////Os__GB2312_display(2, 0, (uchar * )"(预授权)");
				memcpy(aucbuf, (uchar * )"(预授权)",strlen((char*)"(预授权)"));
   			break;
   		case TRANS_VOIDPREAUTH:
			////Os__GB2312_display(2, 0, (uchar * )"(预授权撤销)");
				memcpy(aucbuf, (uchar * )"(预授权撤销)",strlen((char*)"(预授权撤销)"));
			break;
		case TRANS_VOIDPREAUTHFINISH:
			////Os__GB2312_display(2, 0, (uchar * )"(预授权完成撤销)");
				memcpy(aucbuf, (uchar * )"(预授权完成撤销)",strlen((char*)"(预授权完成撤销)"));
			break;
		case TRANS_PREAUTHFINISH:
			////Os__GB2312_display(2, 0, (uchar * )"(预授权完成联机)");
				memcpy(aucbuf, (uchar * )"(预授权完成联机)",strlen((char*)"(预授权完成联机)"));
			break;
		case TRANS_REFUND:
			////Os__GB2312_display(2, 0, (uchar * )" (退货)");
				memcpy(aucbuf, (uchar * )" (退货)",strlen((char*)" (退货)"));
			break;
		case TRANS_TIPADJUST:
		case TRANS_TIPADJUSTOK:
			////Os__GB2312_display(2, 0, (uchar * )" (离线调整)");
				memcpy(aucbuf, (uchar * )" (离线调整)",strlen((char*)" (离线调整)"));
			break;
		case TRANS_OFFPREAUTHFINISH:
			////Os__GB2312_display(2, 0, (uchar * )" (离线结算)");
				memcpy(aucbuf, (uchar * )" (离线结算)",strlen((char*)" (离线结算)"));
			break;
		case TRANS_PREAUTHSETTLE:
			////Os__GB2312_display(2, 0, (uchar * )"(预授权完成离线)");
				memcpy(aucbuf, (uchar * )"(预授权完成离线)",strlen((char*)"(预授权完成离线)"));
			break;
		case TRANS_OFFPURCHASE:
			if((G_NORMALTRANS_ucInputMode == TRANS_INPUTMODE_HB_PUTCARD)
				||(G_NORMALTRANS_ucInputMode == TRANS_INPUTMODE_SAND_PUTCARD))
				////Os__GB2312_display(2, 0, (uchar * )" (qPBOC脱机消费)");
				memcpy(aucbuf, (uchar * )" (qPBOC脱机消费)",strlen((char*)" (qPBOC脱机消费)"));
			else if(G_NORMALTRANS_ucECTrans ==TRANS_EC_PURCHASE)
				////Os__GB2312_display(2, 0, (uchar * )" (EC脱机消费)");
				memcpy(aucbuf, (uchar * )" (EC脱机消费)",strlen((char*)" (EC脱机消费)"));
			else
				////Os__GB2312_display(2, 0, (uchar * )" (脱机消费)");
				memcpy(aucbuf, (uchar * )" (脱机消费)",strlen((char*)" (脱机消费)"));
			break;
		case TRANS_VOIDOFFPURCHASE:
			////Os__GB2312_display(2, 0, (uchar * )" (脱机消费撤销)");
				memcpy(aucbuf, (uchar * )" (脱机消费撤销)",strlen((char*)" (脱机消费撤销)"));
			break;
		case TRANS_QUERYBAL:
			////Os__GB2312_display(2, 0, (uchar * )" (余额查询)");
				memcpy(aucbuf, (uchar * )" (余额查询)",strlen((char*)" (余额查询)"));
			break;
		case TRANS_PREAUTHADD:
			////Os__GB2312_display(2, 0, (uchar * )"(预授权追加)");
				memcpy(aucbuf, (uchar * )"(预授权追加)",strlen((char*)"(预授权追加)"));
			break;
		case TRANS_EC_CASHLOAD:
			////Os__GB2312_display(2, 0, (uchar * )"(现金圈存)");
				memcpy(aucbuf, (uchar * )"(现金圈存)",strlen((char*)"(现金圈存)"));
			break;
		case TRANS_EC_ASSIGNLOAD:
			////Os__GB2312_display(2, 0, (uchar * )"(指定账户圈存)");
				memcpy(aucbuf, (uchar * )"(指定账户圈存)",strlen((char*)"(指定账户圈存)"));
			break;
		case TRANS_EC_UNASSIGNLOAD:
			////Os__GB2312_display(2, 0, (uchar * )"(非指定账户圈存)");
				memcpy(aucbuf, (uchar * )"(非指定账户圈存)",strlen((char*)"(非指定账户圈存)"));
			break;
		case TRANS_MOBILEAUTH:
			////Os__GB2312_display(2, 0, (uchar * )"手机无卡预约消费");
				memcpy(aucbuf, (uchar * )"手机无卡预约消费",strlen((char*)"手机无卡预约消费"));
			break;
		case TRANS_VOIDMOBILEAUTH:
			////Os__GB2312_display(2, 0, (uchar * )"手机预约撤销");
				memcpy(aucbuf, (uchar * )"手机预约撤销",strlen((char*)"手机预约撤销"));
			break;
		case TRANS_MOBILEREFUND:
			////Os__GB2312_display(2, 0, (uchar * )"手机预约退货");
				memcpy(aucbuf, (uchar * )"手机预约退货",strlen((char*)"手机预约退货"));
			break;
		case TRANS_CREDITSALE:
			////Os__GB2312_display(2, 0, (uchar * )"(分期付款消费)");
				memcpy(aucbuf, (uchar * )"(分期付款消费)",strlen((char*)"(分期付款消费)"));
			break;
		case TRANS_CREDITSVOID:
			////Os__GB2312_display(2, 0, (uchar * )"(分期付款撤销)");
				memcpy(aucbuf, (uchar * )"(分期付款撤销)",strlen((char*)"(分期付款撤销)"));
			break;
		case TRANS_CREDITSREFUND:
			////Os__GB2312_display(2, 0, (uchar * )"(分期付款退货)");
				memcpy(aucbuf, (uchar * )"(分期付款退货)",strlen((char*)"(分期付款退货)"));
			break;
		default:
			////Os__GB2312_display(2, 0, (uchar * )" 未知交易类型");
				memcpy(aucbuf, (uchar * )" 未知交易类型",strlen((char*)" 未知交易类型"));
			break;
	}
#ifdef GUI_PROJECT
	...
	memcpy(&ProUiFace.ProToUi.aucLine4[0],"凭证号:",7);
#else
	//Os__GB2312_display(2, 0, (uchar * )"凭证号:");
#endif
...
//	//Os__GB2312_display(2,0,(unsigned char *)"   正在批结算");
//	//Os__GB2312_display(3,0,(unsigned char *)"     请稍候");
	//util_Printf("\n------------------结算不平------正在批结算------1----SERV_SettleBatchEnd-------------\n");
	ISO8583_Clear();
	ISO8583_SetMsgID(320);
	G_NORMALTRANS_ulTraceNumber = DataSave0.ChangeParamData.ulTraceNumber;
	/* Set system trace audit number bit 11 */
...
util_Printf("\n 2. SERV_DispTotal: --------------------------\n");
	...
	memcpy(ProUiFace.ProToUi.aucLine1,"------内卡------",16);
	memcpy(ProUiFace.ProToUi.aucLine2,"消费",4);
#else
	//Os__clr_display(255);
	//Os__GB2312_display(0,0,(uchar *)"------内卡------");
	//Os__GB2312_display(1,0,(unsigned char *)"消费");
#endif
...
//		//Os__clr_display(255);
//		//Os__GB2312_display(0,0,(uchar *)"------内卡------");
//		//Os__GB2312_display(1,0,(unsigned char *)"预授权");
...
util_Printf("\n 4. SERV_DispTotal: --------------------------\n");
	...
	{
#ifdef GUI_PROJECT
		memcpy(ProUiFace.ProToUi.aucLine5,"预授权完成(联机)",strlen((char*)"预授权完成(联机)"));
#else
		//Os__clr_display(255);
		//Os__GB2312_display(0,0,(uchar *)"------内卡------");
		//Os__GB2312_display(1,0, (unsigned char *)"预授权完成(联机)");
#endif
...
//	memset((uchar*)&ProUiFace.ProToUi,0,sizeof(ProUiFace.ProToUi));
//	ProUiFace.ProToUi.uiLines=8;
	memcpy(ProUiFace.ProToUi.aucLine8,"预授权完成(离线)",16);
#else
		//Os__clr_display(255);
		//Os__GB2312_display(0,0,(uchar *)"------内卡------");
		//Os__GB2312_display(1,0,(unsigned char *)"预授权完成(离线)");
#endif
...
util_Printf("\n 6. SERV_DispTotal: --------------------------\n");
	...
	{
#ifdef GUI_PROJECT_tts
		memcpy(ProUiFace.ProToUi.aucLine11,"离线:",5);
#else
		//Os__clr_display(255);
		//Os__GB2312_display(0,0,(uchar *)"------内卡------");
		//Os__GB2312_display(1,0,(unsigned char *)"离线:");
#endif
	...
	{
#ifdef GUI_PROJECT_tts
		memcpy(ProUiFace.ProToUi.aucLine14,"退货:",5);
#else
		//Os__clr_display(255);
		//Os__GB2312_display(0,0,(uchar *)"------内卡------");
		//Os__GB2312_display(1,0,(unsigned char *)"退货:");
#endif
...
util_Printf("\n 7. SERV_DispTotal: --------------------------\n");
	...
	{
#ifdef GUI_PROJECT_tts
	memcpy(ProUiFace.ProToUi.aucLine17,"现金圈存:",9);
#else
		//Os__clr_display(255);
		//Os__GB2312_display(0,0,(uchar *)"------内卡------");
		//Os__GB2312_display(1,0,(unsigned char *)"现金圈存:");
#endif
	...
	{
#ifdef GUI_PROJECT_tts
	memcpy(ProUiFace.ProToUi.aucLine20,"------外卡------",16);
	memcpy(ProUiFace.ProToUi.aucLine21,"消费",4);
#else
		//Os__clr_display(255);
		//Os__GB2312_display(0,0,(uchar *)"------外卡------");
		//Os__GB2312_display(1,0,(unsigned char *)"消费");
#endif
...
//end
	...
	//memset((uchar*)&ProUiFace.ProToUi,0,sizeof(ProUiFace.ProToUi));
	//ProUiFace.ProToUi.uiLines=8;
	memcpy(ProUiFace.ProToUi.aucLine24,"预授权完成(联机)",16);
#else
		//Os__clr_display(255);
		//Os__GB2312_display(0,0,(uchar *)"------外卡------");
		//Os__GB2312_display(1,0,(unsigned char *)"预授权完成(联机)");
	...
	{
#ifdef GUI_PROJECT_tts
	memcpy(ProUiFace.ProToUi.aucLine27,"预授权完成(离线)",16);
#else
		//Os__clr_display(255);
		//Os__GB2312_display(0,0,(uchar *)"------外卡------");
        	//Os__GB2312_display(1,0,(unsigned char *)"预授权完成(离线)");
	...
	{
#ifdef GUI_PROJECT_tts
	memcpy(ProUiFace.ProToUi.aucLine30,"离线:",5);
#else
		//Os__clr_display(255);
		//Os__GB2312_display(0,0,(uchar *)"------外卡------");
        	//Os__GB2312_display(1,0,(unsigned char *)"离线:");
...
// ProUiFace.ProToUi.uiLines=8;
	...
	{
#ifdef GUI_PROJECT_tts
	memcpy(ProUiFace.ProToUi.aucLine33,"离线撤销:",9);
#else
		//Os__clr_display(255);
		//Os__GB2312_display(0,0,(uchar *)"------外卡------");
        	//Os__GB2312_display(1,0,(unsigned char *)"离线撤销:");
#endif
...
util_Printf("\n 9. SERV_DispTotal: --------------------------\n");
   ...
   /*
	//Os__clr_display(255);
	//Os__GB2312_display(0,0,(uchar *)"------外卡------");
	//Os__GB2312_display(1,0,(unsigned char *)"退货:",FONT);
