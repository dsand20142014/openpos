...
#include "ui_pro_cancelauthfinish.h"
#include "msginfo.h"
#include "mythread.h"
#include <QKeyEvent>
...
Pro_CancelAuthFinish::Pro_CancelAuthFinish(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::Pro_CancelAuthFinish)
{
    ui->setupUi(this);
    ...
    xingCnt=0;
    pwdNum=0;
    ui->line1_trace->installEventFilter(this);
...
Pro_CancelAuthFinish::~Pro_CancelAuthFinish()
{
    delete ui;
}
bool Pro_CancelAuthFinish::eventFilter(QObject *target, QEvent *event)
{
     QString str;
     if(target==ui->line1_trace)
     {
...
void Pro_CancelAuthFinish::slottimer1GetCardInfo()
{
    //时钟显示
    unsigned char str[6];
...
int Pro_CancelAuthFinish::gb2312toutf8(char *sourcebuf, size_t sourcelen, char *destbuf, size_t destlen)
{
    iconv_t cd;
...
void Pro_CancelAuthFinish::on_cancelButton_clicked()
{
...
void Pro_CancelAuthFinish::sysEventSlot(int sysEvent)
{
...
void Pro_CancelAuthFinish::destroyWindow(int)
{
...
void Pro_CancelAuthFinish::on_confirmButton_clicked()
{
    ...
    if(ucTransOkFlag == 3){
        ...
        ProUiFace.UiToPro.ucTransType = TRANS_VOIDPREAUTHFINISH;
        ProUiFace.UiToPro.CardInfo.ucInputMode = TRANS_INPUTMODE_SWIPECARD;
        ...
        uiLen = strlen(ui->line2_pwd->text().toAscii());
        if(uiLen){
...
//                desk->showFullScreen();
    ...
    {
        uiTrace= ui->line1_trace->text().toInt(0,10);
        int_asc(G_RUNDATA_aucOriginalTraceNum,6,&uiTrace);
        ucResult =  SAV_SavedTransIndex(1);

        if((ucResult == SUCCESS)&&(ProUiFace.ProToUi.uiLines))
        {
