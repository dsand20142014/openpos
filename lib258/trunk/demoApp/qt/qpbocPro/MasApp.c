...
unsigned char Os__GB2312_display_pp11 (unsigned char line, unsigned char   Column, unsigned char * text)
{
...
unsigned char MASAPP_Event(void)
{
	...
	while(1)
	{
        ...
        Os__GB2312_display11(1,2,(uchar *)"QPBOC TEST ");
        OSMMI_DisplayASCII11(0x30,5,5,(uchar *)"   Ver 1.2");

        emitSignal();

        Os_Wait_Event(KEY_DRV|MFR_DRV,&new_drv,10000);
			...
			if((pxKey->xxdata[1]>='0')&&(pxKey->xxdata[1]<='9'))
            {
               ...
               emitSignal();
                ...
                {                printf("cccccccc\n");
                    ...
                    Os__GB2312_display11(2,0,(PUCHAR)"Put the Card");

                    emitSignal();
                     printf("dddddddddd\n");

                     Os_Wait_Event(KEY_DRV|MFR_DRV,&new_drv1,15000);

                    if(new_drv1.drv_type==KEY_DRV)
                    {

                        pxKey = &new_drv1.drv_data;
                          printf("eeeeeeeee pxKey = %02x \n new_drv1.drv_data =%02x \n",*pxKey , new_drv1.drv_data);
                     ...
                     else if(new_drv1.drv_type==MFR_DRV)
                    {
                        Os__abort_drv(drv_mmi);

                        pxMifare = &new_drv1.drv_data;
						...
						if (ucResult ==QERROR_COLLISION)
						{
                                ...
                                Os__clr_display11(255);
                                Os__GB2312_display11(2,0,"Single Card");
								UTIL_GetKey(1);
						}
                    ...
                    else if(new_drv1.drv_type == DRV_TIMEOUT)
                    {
                        break;
                    }
...
//			if(bDispFlag)
//				Os__GB2312_display11(ucI,0,aucBuff);
//			else
...
UCHAR	EMVTransProcess(QTRANSTYPE enTransType,UINT uiAmount,UINT uiOtherAmount)
{
    ...
    Os__clr_display11(255);
     Os__GB2312_display11(2,0,(PUCHAR)"Processing");
    emitSignal();
...
//			UTIL_WriteConstParamFile(&QConstParam);
		...
		if(!ucResult)
		{
            printf("before QINTIAPP_GPO()\n");
            if(!ucResult)
                ucResult = QINTIAPP_GPO();
            printf("end QINTIAPP_GPO ==%02x\n",ucResult);
	...
	if(!ucResult)
	{
       ...
       printf("~~~~~000000000\n");
        ...
        Os__GB2312_display11(3,0,(PUCHAR)"Processing");
...
//				if(uiAmount>=QReaderParam.ulRCVMReqLimit)
		...
		if(!ucResult||ucResult==QERROR_NEEDARQC)
		{
            ...
            Os__GB2312_display11(2,0,(PUCHAR)"Card Read OK");
            Os__GB2312_display11(3,0,(PUCHAR)"Processing");
          emitSignal();
...
*/			
	...
	//	DIGITAL_Beep(BEEP_TERMINATED);
        ...
        printf("~~~~~~~~~~~trans terminated \n");

        Os__clr_display11(255);
        Os__GB2312_display11(1,0,aucTransResult);
	}
	else
	{
...
//			DIGITAL_Beep(BEEP_DECLINED);
		...
		if(pAOSA)
		{
            ...
            Os__GB2312_display11(2,0,(unsigned char*)"OFFLINE AMOUNT:");
            Os__GB2312_display11(3,0,(unsigned char*)aucBuff);
            emitSignal();
		}
...
void	PrintTransTicket(void)
{
    ...
    Os__clr_display11(255);
    Os__GB2312_display11(1,2,(uchar *)"PRINTING");
    emitSignal();
	memset(aucBuff,0x00,sizeof(aucBuff));
...
UCHAR	ReadICTransLog(void)
{
	...
	while(1)
	{
        ...
        Os__clr_display11(255);
        Os__GB2312_display11(2,2,"Insert Card");

        emitSignal();
         Os_Wait_Event(KEY_DRV|ICC_DRV,&new_drv,15000);
...
//		      &&(pxKey->gen_status != DRV_ENDED));
		...
		//	QConfig.ucTransCurrencyExp = 0x02;
			...
			if(ucResult||uiLen!=2)
			{
                ...
                Os__clr_display11(255);
                Os__GB2312_display11(2,2,"No Log Entry");
                emitSignal();
                Os__xget_key11();
				break;
			}
			...
			for(ucI=1;!ucResult&&ucI<=ucRecordNums;ucI++)
			{
				...
				if(!ucResult&&QICC_CommandResult() == QCARDSWDEF_SUCCESS)
				{
                   ...
                   emitSignal();
                    ...
                    OSMMI_DisplayASCII11(0x30,0,0,aucBuff);
                    OSMMI_DisplayASCII11(0x30,7,0,(unsigned char*)"Any Key To Countinue");
