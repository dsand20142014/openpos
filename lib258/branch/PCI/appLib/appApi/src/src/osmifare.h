...
/**
 * @defgroup 提供对非接卡操作接口函数
 * @author 许小波
 * @brief mifare card operation functions declaration
 * @{
 */
...
/*!  
 *  \def   OSMIFARE_ERR_XXXX
 *  \brief mifare card operation functions return error code,for detail see osmifare.h
 */
...
/*!
 *  \def OSMIFARE_CARD_TYPE_A	0
 *  \brief ISO14443 TYPE A mode
 */
#define OSMIFARE_CARD_TYPE_A				0		//TypeA 模式

/*!
 *  \def OSMIFARE_CARD_TYPE_B	0
 *  \brief ISO14443 TYPE B mode
 */
#define OSMIFARE_CARD_TYPE_B				1		//TypeB 模式

/*!
 *  \def OSMIFARE_CARD_CLASS_MEMERY		0
 *  \brief miafre card class,M1 M4
 */
#define OSMIFARE_CARD_CLASS_MEMERY			0		//M1 M4

/*!
 *  \def OSMIFARE_CARD_CLASS_CPU		1
 *  \brief cpu card class, desfire card, dobule interface card
 */
#define OSMIFARE_CARD_CLASS_CPU				1		//Desfire ,double interface card,mifare prox card

/*!
 *  \def OSMIFARE_AUTH_TYPE_A		0
 *  \brief mifare card mode authentication
 */
#define OSMIFARE_AUTH_TYPE_A				0		//TypeA 标准模式

/*!
 *  \def OSMIFARE_AUTH_TYPE_SH		1
 *  \brief shanghai card mode authentication
 */
#define OSMIFARE_AUTH_TYPE_SH				1		//上海模式

/*!
 *  \def OSMIFARE_KEY_LEN					6
 *  \brief mifare card authentication key length
 */
#define OSMIFARE_KEY_LEN					6

/*!
 *  \def OSMIFARE_KEY_TYPE_A				0
 *  \brief mifare card authentication key A
 */
#define OSMIFARE_KEY_TYPE_A					0		//KeyA

/*!
 *  \def OSMIFARE_KEY_TYPE_B				1
 *  \brief mifare card authentication key B
 */
#define OSMIFARE_KEY_TYPE_B					1		//KeyB

/*!
 *  \def OSMIFARE_REQUEST_REQA				0x26
 *  \brief mifare card standard request card operation code
 */
#define OSMIFARE_REQUEST_REQA				0x26	//request stand

/*!
 *  \def OSMIFARE_REQUEST_WUPA				0x52
 *  \brief mifare card wake up request card operation code
 */
...
/**
 * @fn unsigned char Os__MIFARE_Request(unsigned char ucCardType,unsigned char ucReqMode)
 * @brief  mifare card request card operation function
 * @param  ucCardType:type A card or type B card
 * @param  ucReqMode:standard request card or wake up request card
 *
 * @return succeed return OSMIFARE_ERR_SUCCESS, otherwise return error code
 */
...
/**
 * @fn unsigned char OSMIFARE_Request(unsigned char ucCardType,unsigned char ucReqMode)
 * @brief  mifare card request card operation function
 * @param  ucCardType:type A card or type B card
 * @param  ucReqMode:standard request card or wake up request card
 *
...
/**
 * @fn unsigned char OSMIFARE_Anti( unsigned char ucAntiEn,unsigned char *pucCardId,
 *									 unsigned char *pucCardIdLen,unsigned char *pucCardClass)
 * @brief  mifare card anticollision operation function
 * @param  ucAntiEn:1
 * @param  pucCardId: pointer to save card id buffer
 * @param  pucCardIdLen: pointer to card id length
 * @param  pucCardClass: pointer to card categories, 0 M1 card; 1 CPU card
 *
 * @return succeed return OSMIFARE_ERR_SUCCESS, otherwise return error code
 */
...
/**
 * @fn unsigned char OSMIFARE_Select( unsigned char *pucCardID,unsigned char ucCardIDLen,unsigned char *pucCardClass)
 *									 
 * @brief  mifare card select card operation function
 * @param  pucCardId: pointer to select card id buffer
 * @param  pucCardIdLen: card id length
 * @param  pucCardClass: pointer to card categories
 *
 * @return succeed return OSMIFARE_ERR_SUCCESS, otherwise return error code
 */                             
...
/**
 * @fn unsigned char OSMIFARE_Auth( unsigned char ucAuthType,unsigned char ucSectorIndex,unsigned char ucKeyType, unsigned char *pucKey );
 *									 
 * @brief  mifare card sector authentication operation function
 * @param  ucAuthType: authentication type
 * @param  ucSectorIndex: sector to authenticate
 * @param  ucKeyType: key type
 * @param  ucKeyType: pointer to key buffer
 *
 * @return succeed return OSMIFARE_ERR_SUCCESS, otherwise return error code
 */ 
...
/**
 * @fn unsigned char Os__MIFARE_Active(void)
 * @brief  active cpu card
 *
 * @return succeed return OSMIFARE_ERR_SUCCESS, otherwise return error code
 */
...
/**
 * @fn unsigned char Os__MIFARE_SendRevFrame( unsigned char *pucSendFrame,unsigned int uiSendLen,
 *                                      unsigned char *pucRevFrame,unsigned int *puiRevLen,unsigned int uiTimeDelay )
 * @brief  cpu card exchange data in ISO7816 protocol
 * @param  pucSendFrame: pointer to sand data buffer, data comply with ISO7816 protocol
 * @param  uiSendLen: sand data length
 * @param  pucRevFrame: pointer to receive data buffer, data comply with ISO7816 protocol
 * @param  puiRevLen: pointer to receive data length
 * @param  uiTimeDelay: frame date timeout value, default 0
 *
 * @return succeed return OSMIFARE_ERR_SUCCESS, otherwise return error code
 */
...
/**
 * @fn unsigned char Os__MIFARE_Remove( void )
 * @brief  remove card, function return until card be removed 
 *
 * @return succeed return OSMIFARE_ERR_SUCCESS, otherwise return error code
 */
