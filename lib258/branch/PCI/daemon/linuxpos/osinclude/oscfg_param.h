...
struct _cfg_pow {
	unsigned char  light_off_tm;
    unsigned char  power_off_tm;
	unsigned char  crc[2];	
};
...
extern const struct _cfg_item cfg_item_pow[];

extern const char linuxpos_super_passwd[];

int OSCFG_save_basic(void);
int OSCFG_save_battery(void);
int OSCFG_save_printer(void);
int OSCFG_getVersion(void);
int OSCFG_OS(void);
...
int OSCFG_input_passwd(int , unsigned int , unsigned int , char *);
int OSCFG_is_save(void);
int OSCFG_error_message(int err_value);
extern unsigned char OSCFG_GsmSettingLoad (void);
extern int OSCFG_device_not_exist(void);
extern int OSCFG_config_pwd_init(void);
extern int OSCFG_config_pwd(void);
extern int OSCFG_config_deviceid(void);
#endif
