...
#define  ENCRYPT_MAX_CHECKDATALEN     2

#define  KEY_STARTADDR     5

#if 0
typedef struct
{
	unsigned char	hashtype;                			//ENCRYPT_HASH_SHA1 ,ENCRYPT_HASH_SHA256
	unsigned short	hashlen;                       		//传进来数据的长度,同样也是传出去的数据
	unsigned char	hashindata[ENCRYPT_HASH_INDATA_MAXLEN]; 	//传进来数据
	unsigned char	hashoutdata[ENCRYPT_HASH_SHA256_MAXLEN];	//计算出来的HASH值，一种是20，一种是32
}ENCRYPT_HASH_DATA;
#endif
...
typedef struct
{
	...
	unsigned int random_data[ENCRYPT_RANDOM_DATALEN];
}RANDOM_DATA_GET;


typedef struct 
{
   unsigned char data[ENCRYPT_HASH_INDATA_MAXLEN];
...
typedef struct
{
   ...
   unsigned char       checkdata[ENCRYPT_MAX_CHECKDATALEN];                       //  密钥校验位
}KEY_PROTINFO;
...
unsigned char DYMN_3TDecryptKey(
	...
	unsigned char *pucInData,
	unsigned char *pucOutData);
void gz__xor(unsigned char *a, unsigned char *b, char lg);
...
ENCRYPT_KEY_APPID_DATA        appiddata;
...
int ENCRYPT_HASH_Hardware(unsigned char *aucIndata,unsigned int uiLen,unsigned char ucType,unsigned char *aucOutdata)
{
	...
	if(uiLen > ENCRYPT_HASH_INDATA_MAXLEN)
		return(ENCRYPT_HASH_PARAMERR);
...
#endif
	    if(ucType == ENCRYPT_HASH_SHA1)
	    	memcpy(aucOutdata,(unsigned char*)Keyhashdata.output,ENCRYPT_HASH_SHA1_MAXLEN);
	    else
	    	memcpy(aucOutdata,(unsigned char*)Keyhashdata.output,ENCRYPT_HASH_SHA256_MAXLEN);
	}
	else
	{
...
int ENCRYPT_Get_Random(unsigned char uiLen,unsigned char *pucRandom)
{
	...
	ret =0;
	if(uiLen > ENCRYPT_RANDOM_DATALEN)
		return(ENCRYPT_GETRANDOM_PARAMERR);
...
int ENCRYPT_KEY_KeyCheckApp(ENCRYPT_KEY_APPID_DATA *keyappiddata)
{
	...
	if(keyappiddata->appnumber>ENCRYPT_KEY_MAX_APPNUMBER)
	{
		printf("\n appnumber is error!\n");
		return (-1);
	}
	if((keyappiddata->delflag != ENCRYPT_KEY_DEL_ALL)&&(keyappiddata->delflag != ENCRYPT_KEY_DEL_PART))
	{
		printf("\n delflag is error!\n");
		return (-1);
	}
	if(keyappiddata->delflag == ENCRYPT_KEY_DEL_ALL)
	{
...
int ENCRYPT_Decrypt_Load_Key(ENCRYPT_DRV_DATA *keydrvdata)
{
	...
	if(keydrvdata->cryptflag)
	{
		   memset(aucInput,0,sizeof(aucInput));
		   memset(aucOutput,0,sizeof(aucOutput));
		   aucInput[0] = keydrvdata->reserve[0]+0x01;
		   aucInput[1] = keydrvdata->reserve[2]+0x03;
		   aucInput[2] = keydrvdata->reserve[1]+0x12;
		   aucInput[3] = keydrvdata->reserve[3]+0x23;
		   aucInput[4] = keydrvdata->reserve[2]+0x35;
		   aucInput[5] = keydrvdata->reserve[1]+0x55;
		   aucInput[6] = keydrvdata->reserve[3]+0x29;
		   aucInput[7] = keydrvdata->reserve[0]+0x88;
		   des(aucInput,aucOutput,KEY_DEFAULTVALUE1);
		   if(memcmp(aucOutput,&keydrvdata->reserve[4],8))
		   {
		   	    printf("\ncheck data err,stop do ENCRYPT_Decrypt_Load_Key\n ");
	        	return (-1);
		   }
		   uiTransKeyAppId = keydrvdata->reserve[0]*256+keydrvdata->reserve[1];
		   uiDeTransKeyAppId = keydrvdata->reserve[2]*256+keydrvdata->reserve[3];
	}else
  {
  	   uiTransKeyAppId   = APPLIST_get_appid();
       uiDeTransKeyAppId = APPLIST_get_appid();
  }
  
 /* if((keydrvdata->cryptindex >=KEY_MAX_KEYNUMBER)||(keydrvdata->saveindex >=KEY_MAX_KEYNUMBER))
  {
  	   printf("\nAppId over KEY_MAX_KEYNUMBER\n ");
		   return (-1);
  }
*/
/*
...
*/
	...
	//memcpy(key,keydrvdata->keyvalue,ENCRYPT_MAX_KEYDATA);
	...
	else
	{
		...
		if(keydrvdata->crypttype == ENCRYPT_3DESDOUBLE)
			...
			/*memcpy(keydrvdata->outdata,aucOutput,ENCRYPT_MAX_KEYDATA);
			if(keydrvdata->crypttype == ENCRYPT_3DESDOUBLE)
				keydrvdata->outlen = ENCRYPT_MAX_KEYDATA;
			else
				keydrvdata->outlen = ENCRYPT_MAX_KEYDATA/2;*/
	...
	
{
	...
	if( ucDecryptType==ENCRYPT_3DESDOUBLE_DECRYPT )
		ucKeyType = ENCRYPT_KEYTYPE_DOUBLE;
	else
		ucKeyType = ENCRYPT_KEYTYPE_SINGLE;
...
#endif	
	if(ucKeyType == ENCRYPT_KEYTYPE_DOUBLE)
	{
...
#if MMI_DEB
		printf("\nKEY_keycrypt_imx  ENCRYPT_KEYTYPE_DOUBLE\n");
		for(ucI=0;ucI<ENCRYPT_MAX_KEYDATA;ucI++)
		{
			printf(" %02x",aucOutKey[ucI]);
		} 
...
int KEY_prot_put_key(unsigned char index,unsigned char *key)
{
	...
	int protlen,res;
	struct stat fst;
	unsigned char aucbuf[ENCRYPT_MAX_KEYDATA/2+1];
	...
	memset(aucbuf,0,sizeof(aucbuf));
 	des(KEY_DEFAULTVALUE1,aucbuf,Keyprotdata.keyvalue);
 	Keyprotdata.checkdata[0] = aucbuf[0];
 	memset(aucbuf,0,sizeof(aucbuf));
 	des(KEY_DEFAULTVALUE1,aucbuf,&Keyprotdata.keyvalue[ENCRYPT_MAX_KEYDATA/2]);
 	Keyprotdata.checkdata[1] = aucbuf[0]; 
	if ((res = write(fd, &Keyprotdata, protlen)) != protlen) 
	{
		printf("write protect is error\n ");
		retval = -1;
	...
	else
	{
		...
		printf("\nindex=%02x,Keyprotdata.keyindex=%02x",index,Keyprotdata.keyindex);
 	    ...
 	    if(Keyprotdata.keyindex == index)
 	    {
 	    	  ...
 	    	  memset(aucbuf,0,sizeof(aucbuf));
 	    	  des(KEY_DEFAULTVALUE1,aucbuf,Keyprotdata.keyvalue); 
 	    	  if(aucbuf[0] == Keyprotdata.checkdata[0])
 	    	  {
 	    	  	   memset(aucbuf,0,sizeof(aucbuf));
 	    	       des(KEY_DEFAULTVALUE1,aucbuf,&Keyprotdata.keyvalue[ENCRYPT_MAX_KEYDATA/2]);  
 	    	       if(aucbuf[0] != Keyprotdata.checkdata[1])
 	    	       {
 	    	       	  memcpy(key,KEY_DEFAULTVALUE1,ENCRYPT_MAX_KEYDATA/2);
									memcpy(&key[ENCRYPT_MAX_KEYDATA/2],KEY_DEFAULTVALUE2,ENCRYPT_MAX_KEYDATA/2);	
									goto exit; 
...
/*	if(index>(KEY_MAX_KEYNUMBER-1))
	...
	//lseek(fd,0,SEEK_SET);	
	keylen = sizeof(KEY_DATAINFO);
	lseek(fd,keylen*KEY_STARTADDR,SEEK_SET);
	for(i=KEY_STARTADDR;i<KEY_MAX_KEYNUMBER;i++)
  {
  	  memset(&Keydatainfo,0,sizeof(KEY_DATAINFO));
		  if ((res = read(fd,&Keydatainfo,keylen)) ==  keylen)
		  {
		  	  if(Keydatainfo.keyvalid)
		  	  {
		  	  	  if((Keydatainfo.keyvalid == KEY_SINGLE_VALUE)||(Keydatainfo.keyvalid == KEY_DOUBLE_VALUE))
				  	  {
			  	  	    if(Keydatainfo.index == index)
			  	  	    {
			  	  	    	    ucFlag = 1;
			  	  	    	    break;
			  	  	    }
  ...
  {
#if MMI_DEB
	printf("\nKEY_cipher_put_key index=%d,i=%d",index,i);
#endif
  	    lseek(fd,keylen*i,SEEK_SET);	
				memset(&Keydatainfo,0,sizeof(KEY_DATAINFO));
				Keydatainfo.index = index;
				memset(aucbuf,0,sizeof(aucbuf));
      	des(KEY_DEFAULTVALUE1,aucbuf,key);
 	      Keydatainfo.checkdata[0] = aucbuf[0];
				if(ucDoubleFlag == KEY_DOUBLE_VALUE)
				{
					Keydatainfo.keyvalid = KEY_DOUBLE_VALUE;
					memcpy(&Keydatainfo.keyvalue,key,ENCRYPT_MAX_KEYDATA);
					memset(aucbuf,0,sizeof(aucbuf));
      	  des(KEY_DEFAULTVALUE1,aucbuf,&key[ENCRYPT_MAX_KEYDATA/2]);
 	        Keydatainfo.checkdata[1] = aucbuf[0];
				}
				else
				{ 
...
/*	if(index>(KEY_MAX_KEYNUMBER-1))
  ...
  memcpy(key,KEY_DEFAULTVALUE1,ENCRYPT_MAX_KEYDATA/2);
	...
	else
	{
		  keylen = sizeof(KEY_DATAINFO);
		  lseek(fd,keylen*KEY_STARTADDR,SEEK_SET);
		  for(uiI=KEY_STARTADDR;uiI<KEY_MAX_KEYNUMBER;uiI++)
		  {
		  	 memset(&Keydatainfo,0,sizeof(KEY_DATAINFO));
		  	 if ((res = read(fd,&Keydatainfo,keylen)) !=  keylen)
		  	 {
		...
		printf("\nindex=%02x,Keydatainfo.keyvalid=%02x",index,Keydatainfo.keyvalid);
		  	 	   	    	  	    ...
		  	 	   	    	  	    if(ucDoubleFlag == KEY_DOUBLE_VALUE)
														{
															memset(aucbuf,0,sizeof(aucbuf));
	 	    	                    des(KEY_DEFAULTVALUE1,aucbuf,Keydatainfo.keyvalue); 
	 	    	                    if(aucbuf[0] != Keydatainfo.checkdata[0])
	 	    	                    {
	 	    	                    	  // memcpy(key,KEY_DEFAULTVALUE1,ENCRYPT_MAX_KEYDATA/2);
	 	    	                    	  // memcpy(&key[ENCRYPT_MAX_KEYDATA/2],KEY_DEFAULTVALUE2,ENCRYPT_MAX_KEYDATA/2);	
	 	    	                    	   break;
														...
														//	memcpy(&key[ENCRYPT_MAX_KEYDATA/2],KEY_DEFAULTVALUE2,ENCRYPT_MAX_KEYDATA/2);	
															memset(aucbuf,0,sizeof(aucbuf));
	 	    	                    des(KEY_DEFAULTVALUE1,aucbuf,&Keydatainfo.keyvalue[ENCRYPT_MAX_KEYDATA/2]); 
	 	    	                    if(aucbuf[0] != Keydatainfo.checkdata[1])
	 	    	                    {	 	  	 	    	                    	     	                    	  
	 	    	                    	   break;
	 	    	                    }
...
int ENCRYPT_LoadSpecialKey(unsigned char ucKeytype,unsigned char ucCrypttype,unsigned char ucDoubleflag,unsigned char *aucIndata,unsigned char ucChecktype)
{
	    ...
	    int res,keylen;
	    int retval = 0;
	    unsigned char key[ENCRYPT_MAX_KEYDATA],aucbuf[ENCRYPT_MAX_KEYDATA+1],auctemp[ENCRYPT_MAX_KEYDATA+1];
	    ...
	    i = ENCRYPT_Creat_Directory();
			...
			if(ucKeytype>=KEY_STARTADDR)
			   return (-1);
			
			appid   = APPLIST_get_appid();
			memset(aucAppId,0,sizeof(aucAppId));
			short_asc(aucAppId,5,&appid);
			memset(buf,0,sizeof(buf));
			strcpy(buf,KEY_INFO_FILE_HEAD);
	...
	printf("\nbuf=%s",buf);
     ...
     if(stat(buf,&fst) < 0)
		...
		keylen = sizeof(KEY_DATAINFO);	
		if(ucKeytype != ENCRYPT_MASTERKEY)
		{
		     lseek(fd,0,SEEK_SET);
		     memset(&Keydatainfo,0,sizeof(KEY_DATAINFO));
		     if ((res = read(fd,&Keydatainfo,keylen)) !=  keylen)
		  	 ...
		  	 {		  	     		  	     
	 	    	   if((Keydatainfo.keyvalid != KEY_DOUBLE_VALUE)&&(Keydatainfo.keyvalid != KEY_SINGLE_VALUE)) 
	 	    	   {
	 	    	        memcpy(key,KEY_DEFAULTVALUE1,ENCRYPT_MAX_KEYDATA/2);
						      memcpy(&key[ENCRYPT_MAX_KEYDATA/2],KEY_DEFAULTVALUE2,ENCRYPT_MAX_KEYDATA/2);	 
	 	    	   }else
	 	    	   {
	 	    	       memcpy(key,Keydatainfo.keyvalue,ENCRYPT_MAX_KEYDATA);
	 	    	       memset(aucbuf,0,sizeof(aucbuf));
			 	    	   des(KEY_DEFAULTVALUE1,aucbuf,Keydatainfo.keyvalue);
			 	    	   if(aucbuf[0] != Keydatainfo.checkdata[0])
			 	    	       memcpy(key,KEY_DEFAULTVALUE1,ENCRYPT_MAX_KEYDATA/2);			 	    	  
		  	 ...
		  	 if( 0 != KEY_keycrypt_imx(key,SCC_DECRYPT,ENCRYPT_MAX_KEYDATA))
				{
					  retval = -1;
				    goto exit;
				}
   ...
   {
        case ENCRYPT_DES_SINGLE_DECRYPT:
             desm1(aucIndata,aucbuf,key);
             break;
        case ENCRYPT_3DES_SINGLE_DECRYPT:
             desm1(aucIndata,aucbuf,key);
             des(aucbuf,aucbuf,&key[ENCRYPT_MAX_KEYDATA/2]);
             desm1(aucbuf,aucbuf,key);
		         break;
		    default:
		         break;
   }
   if(ucDoubleflag == ENCRYPT_KEYTYPE_DOUBLE)
   {
       switch(ucCrypttype)
		   {
		        case ENCRYPT_DES_SINGLE_DECRYPT:
		             desm1(&aucIndata[ENCRYPT_MAX_KEYDATA/2],&aucbuf[ENCRYPT_MAX_KEYDATA/2],key);
		             break;
		        case ENCRYPT_3DES_SINGLE_DECRYPT:
		             desm1(&aucIndata[ENCRYPT_MAX_KEYDATA/2],&aucbuf[ENCRYPT_MAX_KEYDATA/2],key);
		             des(&aucbuf[ENCRYPT_MAX_KEYDATA/2],&aucbuf[ENCRYPT_MAX_KEYDATA/2],&key[ENCRYPT_MAX_KEYDATA/2]);
		             desm1(&aucbuf[ENCRYPT_MAX_KEYDATA/2],&aucbuf[ENCRYPT_MAX_KEYDATA/2],key);
				         break;
				    default:
				         break;
		   } 
    ...
    if(ucDoubleflag == ENCRYPT_KEYTYPE_DOUBLE)
	  	   uilen =   ENCRYPT_MAX_KEYDATA;
	  else
	       uilen =   ENCRYPT_MAX_KEYDATA/2;  	   
    memset(key,0,sizeof(key));
    switch(ucKeytype)
		{		   
		    case ENCRYPT_PINKEY:
		    case ENCRYPT_MACKEY:
		         switch(ucChecktype)
		         {
		              case ENCRYPT_CHECKTYPE_ZGYL_S:
		                    memset(auctemp,0,sizeof(auctemp));
		                    des(auctemp,auctemp,aucbuf);
		                    if(memcmp(auctemp,&aucIndata[ENCRYPT_MAX_KEYDATA/2],4))
		                    {
		                          retval = -1;
				                      goto exit;
		                    }
		                    break;
		              case ENCRYPT_CHECKTYPE_ZGYL_D:
		                    memset(auctemp,0,sizeof(auctemp));
		                    des(auctemp,auctemp,aucbuf);
		                    desm1(auctemp,auctemp,&aucbuf[ENCRYPT_MAX_KEYDATA/2]);
		                    des(auctemp,auctemp,aucbuf);
		                    if(memcmp(auctemp,&aucIndata[ENCRYPT_MAX_KEYDATA],4))
		                    {
		                          retval = -1;
				                      goto exit;
		                    }	                    
		         ...
		         memcpy(key,aucbuf,uilen);
		         break;		   
		    default:
		         memcpy(key,aucIndata,uilen);
		         break;
		}		
#if MMI_DEB
	printf("\nKEY_keycrypt_imx in uilen =%d\n",uilen);
	for(i=0;i<uilen;i++)
	   printf("%02x ",key[i]); 
#endif			
 
	    if( 0 != KEY_keycrypt_imx(key,SCC_ENCRYPT,uilen))
			{
				  retval = -1;
				  goto exit;
			}   
#if MMI_DEB
	printf("\nKEY_keycrypt_imx out uilen =%d\n",uilen);
	for(i=0;i<uilen;i++)
	   printf("%02x ",key[i]); 
#endif	
			
		lseek(fd,keylen*ucKeytype,SEEK_SET);
		memset(&Keydatainfo,0,sizeof(KEY_DATAINFO));
		Keydatainfo.index = ucKeytype;
		memcpy(&Keydatainfo.keyvalue,key,uilen);
		memset(aucbuf,0,sizeof(aucbuf));
  	des(KEY_DEFAULTVALUE1,aucbuf,key);
    Keydatainfo.checkdata[0] = aucbuf[0];
		if(ucDoubleflag == ENCRYPT_KEYTYPE_DOUBLE)
...
int ENCRYPT_MAC_Process(unsigned char *aucIndata,unsigned int uiInlen,unsigned char uctype,unsigned char *aucOutdata)
{
	...
	printf("\nbuf=%s",buf);
     ...
     if(stat(buf,&fst) < 0)
		...
		keylen = sizeof(KEY_DATAINFO);	
		lseek(fd,keylen*ENCRYPT_MACKEY,SEEK_SET);
		memset(&Keydatainfo,0,sizeof(KEY_DATAINFO));
		if ((res = read(fd,&Keydatainfo,keylen)) !=  keylen)
  	 {
  	 	  fprintf(stderr,"[%s:%d] %02x\n",__func__,__LINE__,errno);
		...
		switch(uctype)
		{
		    case ENCRYPT_CHECKTYPE_ZGYL_S:
		          pucPtr = aucIndata;
							...
							{
								uiInlen = min(8,aucIndata+uiInlen-pucPtr);
								for(uiJ=0;uiJ<uiInlen;uiJ++)
								{
						  ...
						  des(aucHexData,aucHexData,key);							
							hex_asc(aucAscData,aucHexData,16);											
							 memcpy(aucOutdata,aucAscData,8);
		         break;
		    default:
		         break;
		}
...
int ENCRYPT_PIN_Process(unsigned char *aucIndata,unsigned long ulAmount,unsigned char uctype,unsigned char *aucOutdata,unsigned short timeout)
{
	    ...
	    unsigned char key[ENCRYPT_MAX_KEYDATA],aucbuf[ENCRYPT_MAX_KEYDATA+1],auctemp[ENCRYPT_MAX_KEYDATA+1];
	    unsigned int uilen=0;  
	    unsigned char aucPINBuf[ENCRYPT_MAX_KEYDATA+1],aucPIN[13],aucCardPan[ENCRYPT_MAX_KEYDATA/2+1];
	    unsigned char *pucPtr;
	    unsigned short uiI,uiJ;
	    unsigned int inputlen=0;
	    unsigned char displaybuf[25];
	    int  fd;
	...
	printf("\nbuf=%s",buf);
     ...
     if(stat(buf,&fst) < 0)
		...
		keylen = sizeof(KEY_DATAINFO);	
		lseek(fd,keylen*ENCRYPT_PINKEY,SEEK_SET);
		memset(&Keydatainfo,0,sizeof(KEY_DATAINFO));
		if ((res = read(fd,&Keydatainfo,keylen)) !=  keylen)
  	 {
  	 	  fprintf(stderr,"[%s:%d] %02x\n",__func__,__LINE__,errno);
	...
	for(i=0;i<ENCRYPT_MAX_KEYDATA;i++)
  {
    ...
    memcpy(displaybuf,"金额: ",6);
    UTIL_Form_Montant(&displaybuf[6],ulAmount,2);
    retval = Os__get_PIN(displaybuf,aucPIN,timeout);
    if(retval < 0)
    {
    	   goto exit;
    }
		...
		switch(uctype)
		{
		    case ENCRYPT_CHECKTYPE_ZGYL_S:
		         ...
		         if(inputlen)
		         {
		            ...
		            aucPINBuf[0] = '0';
		            aucPINBuf[1] = inputlen+'0';
		            memcpy(&aucPINBuf[2],aucPIN,inputlen);
		            asc_hex(aucPIN,8,aucPINBuf,16);
			          memset(aucCardPan,0,sizeof(aucCardPan));
							  memcpy(&aucCardPan[2],aucIndata,6);
		            gz__xor(aucCardPan,aucPIN,8);
		            des(aucPIN,&aucOutdata[1],key);
		            aucOutdata[0] = 8;
		         }else
		         {
		             aucOutdata[0] = 0;
