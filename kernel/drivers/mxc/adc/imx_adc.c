...
extern void gpio258_buttonpower_init(void);
extern void gpio258_buttonpower_csdown(void);
extern void gpio258_buttonpower_csup(void);
extern int  gpio258_getadcpwpin(void);
extern int gpio258_getwifipwpin(void);
...
void imx_tsc_init(void)
{
	...
	reg = TSC_4WIRE_TOUCH_DETECT;//FIXME:
	__raw_writel(reg, tsc_base + TICR);//FIXME:Here may cause the adc value jump.
				    ...
				    int wait_tsi)
{
	...
	if (wait_tsi) {
		/* Config idle for 4-wire */
		reg = TSC_4WIRE_PRECHARGE|0x01<<20;
        	__raw_writel(reg, tsc_base + TICR);
		...
		{
			if(__raw_readl(tsc_base + TCQSR)==0x00002000)
				break;
			continue;
		}
		/* stop the conversion */
		...
		while (!(__raw_readl(tsc_base + TCQSR) & CQSR_EOQ))
        {
            if(__raw_readl(tsc_base + TCQSR)&CQSR_EMPT)
            {
                count++;
                if(count>5000000)
                {
...
static int imx_adc_filter(struct t_touch_screen *ts_curr)
{
	...
	{
		//pr_alert("imx_adc_filter:not valid.");
		return -1;
	}
	...
	if ((ydiff1 > DELTA_Y_MAX) ||
		...
		//pr_alert("imx_adc_filter: Ret pos 1\n");
		return -1;
	}
	...
	if ((xdiff1 > DELTA_X_MAX) ||
		...
		//pr_alert("imx_adc_filter: Ret pos 2\n");
		return -1;
	}
	...
	if (filt_count < FILTER_MIN_DELAY) {
		...
		if (abs(sample_sumx - (prev_x[0] + prev_x[1])) >
		    (DELTA_X_MAX * 16)) {
			//pr_alert("imx_adc_filter: : Ret pos 3\n");
			return -1;
		}
		if (abs(sample_sumy - (prev_y[0] + prev_y[1])) >
		    (DELTA_Y_MAX * 16)) {
			//pr_alert("imx_adc_filter: : Ret pos 4\n");
			return -1;
		}
		sample_sumy /= 2;
		sample_sumx /= 2;
...
int imx_adc_init(void)
{
	...
	/* Internal reference */
	tsc_intref_enable();//if use 2.5v intref,open this.
...
//	printk("\r\n [%s,%d]\r\n",__func__,__LINE__);
	...
	case IMX_ADC_CONVERT:
		...
		if (convert_param->channel == GER_PURPOSE_ADC0)	gpio258_buttonpower_csup();
		...
		{
		case GER_PURPOSE_ADC0:
			gpio258_buttonpower_csdown();
			break;
		case GER_PURPOSE_ADC1:
			//if (!(readb(cpld_adc) & 0x3))
			//	memset(convert_param->result,0,sizeof(convert_param->result));
			break;
		}
		...
		if (convert_param->channel == GER_PURPOSE_ADC0)	gpio258_buttonpower_csdown();
		kfree(convert_param);
		break;
	...
	case IMX_ADC_BUTTONPOWER:
		//printk("\r\n [%s,%d]\r\n",__func__,__LINE__);
		pin_pw = gpio258_getadcpwpin();
		//printk(" [%s,%d] pin_pw:0x%x\r\n",__func__,__LINE__,pin_pw);
		copy_to_user((u32 *)arg, &pin_pw, sizeof(u32));
		gpio258_buttonpower_csup();
		break;

	case IMX_ADC_LITHIUMPOWER:
		//	imx_adc_convert(GER_PURPOSE_ADC1,&adc);
		//	printk("\r\n [%s,%d] adc:0x%x\r\n",__func__,__LINE__,adc);
		//	copy_to_user((u16 *)arg, &adc, sizeof(u16));
		gpio258_buttonpower_csdown();
			break;

	case IMX_ADC_PRINTEROVERHEAD:
			imx_adc_convert(GER_PURPOSE_ADC2,&adc);
	//		printk("\r\n [%s,%d] adc:0x%x\r\n",__func__,__LINE__,adc);
			copy_to_user((u16 *)arg, &adc, sizeof(u16));
			break;
	case IMX_ADC_ADPTER :
...
static  ssize_t imx_adc_read(struct file *filep, char __user *buf, size_t count, loff_t *offset)
{
	    unsigned char  battery_cpld =readb(cpld_adc);
	    //printk("\r\n [%s,%d] battery_cpld:0x%x\r\n",__func__,__LINE__,battery_cpld);
	    copy_to_user(buf,&battery_cpld,1);
	    return 0;
}
...
//#define  BATTERY_ADDR  0xa0300000
...
static int imx_adc_module_probe(struct platform_device *pdev)
{
	...
	cpld_adc = ioremap_nocache(BATTERY_ADDR, 1);
	...
	device_init_wakeup(&pdev->dev, 0);  //xu.xb  remove touchscreen wakeup system   
