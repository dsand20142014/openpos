...
#define di_debug(fmt, arg...)  do{if(dbg)os_printk(KERN_DEBUG fmt,##arg);}while(0)

#define di_info(fmt, arg...)       do{if(dbg)os_printk(KERN_INFO fmt,##arg);}while(0)
...
//extern struct keycrypt_struct *cryptptr;
 struct fasync_struct *dryice_async_queue=NULL;
/*
...
{
	di_debug("FUNC: %s\n", __func__);
	di_debug("todo.status_start =%02x\n",todo.status);
...
unsigned int dryice_get_register(unsigned char offset)
{
	return(di_read((int)offset));
}

EXTERN_SYMBOL(dryice_get_register);

unsigned int dryice_set_register(unsigned char offset,unsigned int uiValue)
{
	return((unsigned int)di_write_loop(uiValue,(int)offset));
}
EXTERN_SYMBOL(dryice_set_register);

unsigned int dryice_get_security_status(void)
{
	unsigned int ucStatus;
	unsigned int  dsr;

	 ucStatus = 0;
	 ucStatus = di_read(DSR);
	 dsr = di_read(DGPR);
   	 di_info("DGPR1=%08x\n",dsr);
	 if(dsr != SECURITY_FLAG)	 	
	 	ucStatus |= POWEROFF_TAMPER;
	return(ucStatus);
}
EXTERN_SYMBOL(dryice_get_security_status);
/*  只能在第一次开机的时候调用以清除dryice的状态标识， */
/*后续使用过程中只有设备触发安全状态后在维修时才可调用*/
unsigned int dryice_initial_security_status(void) 
{
	unsigned int dsr;
	unsigned int ret;
	
	 dsr = 0;
	 ret = dryice_set_register(DGPR,SECURITY_FLAG);
	 if(ret)
	 	return(ret);
		...
		if (dsr & DSR_SVF)
			di_write_loop_goto(DSR_SVF, DSR, ret, err_write);

		/* clear tamper detect flags */
		if (dsr & DSR_TAMPER_BITS)
			di_write_loop_goto(DSR_TAMPER_BITS, DSR, ret, err_write);

		/* initialize timers */
		di_write_loop_goto(0, DTCLR, ret, err_write);
		di_write_loop_goto(0, DTCMR, ret, err_write);
		di_write_loop_goto(0, DMCR, ret, err_write);

		/* clear non-valid flag */
		di_write_loop_goto(DSR_NVF, DSR, ret, err_write);//must be last cleared 
	}
	 dsr = di_read(DSR);
	 di_info("DSR = %08x!\n",dsr); 	 
...
EXTERN_SYMBOL(dryice_initial_security_status);
...
EXTERN_SYMBOL(dryice_async_queue);
OS_DEV_ISR(dryice_sec_irq)
{
   unsigned int dryice_status;
   printk("the device is attacked!\n");  
   dryice_status = di_read(DSR);
   printk("dryice_status = %08x!\n",dryice_status); 
   dryice_status = di_read(DIER);
    printk("dryice_IER = %08x!\n",dryice_status); 
  if(dryice_async_queue)
   	{
          printk("asyn signal to task!\n");
          kill_fasync(&dryice_async_queue, SIGIO, POLL_IN);
   	}
  di_write(di_read(DIER) & ~DIER_SVIE, DIER);  //mask security interrupt
   return(0);
}
...
{
	di_return_t rc = 0;
	unsigned int dcr_state;
	unsigned int  dgpr;
	//unsigned int  uiDTCLR,uiDTCMR;
	//unsigned int cpsr;
	//unsigned char ucI;
	...
	if (rc)
		...
		di->irq_norm.set = 1;
        rc = request_irq(di->irq_sec.irq, dryice_sec_irq, IRQF_SHARED,
				"dry_ice_sec", di);
	if (rc)
		goto err_irqs;
	else
	...
	dryice_initial_security_status();  //clear  voltage, clock ...tamper flag,only called once,for debug 
	/*  read dryice status*/	
	dcr_state = di_read(DSR);
	di_info("DSR1=%08x\n",dcr_state);
	
	/* set tamper events we are interested in watching */
	di_write_loop_goto(0,
#if 0
						DTCR_TTE|DTCR_SAIE|DTCR_VTE |DTCR_CTE|
					 // DTCR_WGFE|//DTCR_EBE|DTCR_SAOE| 
					   DTCR_WTE | DTCR_ETBE | DTCR_ETAE,//|
#endif
					// ((0x1F<< DTCR_WTGF_SHIFT)&DTCR_WTGF_MASK)|
					 //((0x1F << DTCR_ETGFA_SHIFT)&DTCR_ETGFA_MASK)|
					 //((0x1F << DTCR_ETGFB_SHIFT)&DTCR_ETGFB_MASK),					 
					 DTCR, rc, err_write); 

       di_info("DTCR=%08x\n",di_read(DTCR));
       di_write_loop(DIER_SVIE, DIER);  
       di_info("dryice_IER = %08x\n",di_read(DIER)); 
...
di_return_t dryice_check_key(di_key_t *key)
{
	...
	dksr = di_read(DKSR);
		...
		dksr = DKSR_IIM_KEY;
		rc = DI_ERR_STATE;		
	} else if (dksr == DKSR_RAND_KEY || dksr == DKSR_RAND_XOR_IIM_KEY) {   //error modify by ljm 2011/10/12 replace  DI_KEY_RK DI_KEY_FRK with DKSR_RAND_KEY DKSR_RAND_XOR_IIM_KEY
