...
//extern int hm2057_reg_read(unsigned short reg);
...
static int csi_allocate_frame_buf(cam_data *cam, int count)
{
	...
	for (i = 0; i < count; i++) {
		...
		pr_debug("frame[%02x]=%08x",i,cam->frame[i].buffer.m.offset);
		cam->frame[i].index = i;
...
static int csi_v4l_dqueue(cam_data *cam, struct v4l2_buffer *buf)
{
	...
	//  pr_debug("frame->index=%08x\n",frame->index);
	buf->bytesused = cam->v2f.fmt.pix.sizeimage;
	buf->index = frame->index;
	buf->flags = frame->buffer.flags;
	
	//pr_debug("offset=%08x userptr=%08x\n",cam->frame[frame->index].buffer.m.offset,cam->frame[frame->index].buffer.m.userptr);
	buf->m = cam->frame[frame->index].buffer.m;	
	     //   pr_debug("retvalcsi_dqueue=%02x!\n",retval);
	return retval;
}
...
static int csi_v4l_open(struct file *file)
{
	...
	if (cam->open_count++ == 0) {
		...
		vidioc_int_s_power(cam->sensor, 1);   //power on 
		vidioc_int_dev_init(cam->sensor);
		vidioc_int_init(cam->sensor);
	}

    if (!camera_is_exist()) {
        pr_alert("ERROR: camera is not exist.");
	    csi_v4l_close(file);
        up(&cam->busy_lock);
        return -EIO;
    }
...
static int csi_v4l_close(struct file *file)
{
	...
	if (--cam->open_count == 0) {
		...
		vidioc_int_s_power(cam->sensor, 0);  //power down camera
	}
			    ...
			    unsigned int ioctlnr, void *arg)
{
	...
	case VIDIOC_G_OUTPUT:{
		unsigned char *ucValue = arg;		
		vidioc_int_g_brightness(cam->sensor,ucValue);
		break;
	}
	case VIDIOC_S_CTRL:
	case VIDIOC_G_STD:
...
static void init_camera_struct(cam_data *cam)
{
	...
	*(cam->video_dev) = csi_v4l_template;
	...
	cam->v2f.fmt.pix.sizeimage = 240 * 320 * 2;
	cam->v2f.fmt.pix.bytesperline = 320 * 2;
	cam->v2f.fmt.pix.width = 320;
	cam->v2f.fmt.pix.height = 240;
	...
	cam->win.w.top = 0;
	cam->still_counter = 0;
	cam->low_power = false; //add by ljm
...
static int csi_v4l2_resume(struct platform_device *pdev)
{
	...
	stop_preview(cam);  //add by ljm
        pr_debug("overlay_on = %02x\n",cam->overlay_on);
	if (cam->overlay_on == true)
		start_preview(cam);
