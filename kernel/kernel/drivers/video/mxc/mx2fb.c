...
/***************** sand_bootsplash begin ******************/
#include <linux/timer.h>

//#define SAND_STOP_SPLASH _IOW('M', 10, struct mx2fb_gbl_alpha)
#define SAND_STOP_SPLASH _IOW('M', 168, int)
...
#define COLOR_GRAY	(16<<11|32<<5|16)
#define COLOR_DARKGRAY	(2<<11|4<<5|2)
#define COLOR_PURPLE (COLOR_BLUE|COLOR_RED)
#define COLOR_YELLOW (COLOR_RED|COLOR_GREEN)
#define COLOR_CYAN	(COLOR_BLUE|COLOR_GREEN)

static inline sand_point sand_get_real_point(sand_point point)
{
		sand_point real_point;
		real_point.x=point.y;
		real_point.y=240-point.x;
		return real_point;
}

static inline void sand_draw_point(sand_point point, unsigned short color)
{
		iowrite16(color,((unsigned short*)(screen_base))+320*point.y+point.x);
}

static inline void sand_draw_rect(sand_point left_top,sand_point right_buttom, unsigned short color)
{
		int i,j;
		if(left_top.x<right_buttom.x)
		{
				for(i=left_top.x;i<=right_buttom.x;i++)
				{
						if(left_top.y<right_buttom.y)
						{
								for(j=left_top.y;j<=right_buttom.y;j++)
								{
										iowrite16(color,((unsigned short*)(screen_base))+320*j+i);
								}
						}
						else
						{
...
#define DELTA_T (4)
#define SPLASH_TIME (60*10) //秒
#define MAX_COUNT (SPLASH_TIME*100/DELTA_T)
#define REAL_Y (290)
#define SQUAL_WIDTH (8)
#define SQUAL_HEIGHT (10)
#define SQUAL_NUM (240/SQUAL_WIDTH)

static void sand_splash_fn(void)
{
		/** splash animation here **/
		sand_point rect_lt,rect_rb;
		int i,j;
		int c;	

		for(i=0;i<SQUAL_NUM;i++)
		{
				rect_lt.x=REAL_Y;
				rect_lt.y=(i%SQUAL_NUM)*SQUAL_WIDTH;
				rect_rb.x=REAL_Y+SQUAL_HEIGHT-1;
				rect_rb.y=(i%SQUAL_NUM)*SQUAL_WIDTH+SQUAL_WIDTH-1;
				c=(i+count)%SQUAL_NUM;
				if(c<(SQUAL_NUM/2))
				{
					j=c+1;
					sand_draw_rect(rect_lt,rect_rb,COLOR_DARKBLUE|j<<11|j<<6);
				}
				else
				{
					j=SQUAL_NUM-c;
...
static void begin_splash(void)
{
		...
		t_list.function=sand_splash_fn;
		add_timer(&t_list);
}

static void stop_splash(void)
{
	del_timer(&t_list);
}
/**************** sand_bootsplash end ****************/
...
static void _update_lcdc(struct fb_info *info)
{
		...
		pcr |= (var->sync & FB_SYNC_VSYNC_LOW_ACT) ? 0x00800000 : 0;
		pcr |= (var->sync & FB_SYNC_HSYNC_LOW_ACT) ? 0x00400000 : 0;
...
{
#if 0
	unsigned char * reg_lcd_2 = ioremap_nocache(0xA0500001, 1);
    unsigned char count;
    while(1)
    {
        if(count==0)
        ...
        *reg_lcd_2=count;
        if(*reg_lcd_2!=count)
        {
            pr_alert("********reg_lcd_2=%02X, count=%02X", *reg_lcd_2, count);
            break;
        }
