...
#define scc_debug(fmt, arg...)  do{if(dbg)os_printk(KERN_DEBUG fmt,##arg);}while(0)

#define scc_info(fmt, arg...)       do{if(dbg)os_printk(KERN_INFO fmt,##arg);}while(0)
/*!
...
static int scc_init(void)
{
	...
	printk(KERN_ALERT"SCC module enter!\n");
	...
	/* ! STATUS_INITIAL */
	scc_debug("SCC: Driver Status is %s\n",
		 ...
		 "UNKNOWN");
	printk(KERN_ALERT"SCC init return value=%d\n",return_value);
...
static void scc_cleanup(void)
{
	...
	scc_debug("SCC driver cleaned up.\n");
...

{
	...
	if(smn_state == SMN_STATE_NON_SECURE || smn_state == SMN_STATE_FAIL)
		return return_code;
	...
	if (scc_availability != SCC_STATUS_OK
		...
		scc_debug
		    ("SCC: scc_crypt() count_in_bytes_ok = %d; data_in_ok = %d;"
		     ...
		     !(crypto_mode == SCC_CBC_MODE && init_vector == NULL));
		scc_debug("SCC: scc_crypt() mode_ok=%d; direction_ok=%d;"
		...
		scc_debug("SCC: scc_crypt() detected bad argument\n");
	} else {
...
OS_DEV_ISR(scc_irq)
{
	...
	int smn_irq = INT_SCC_SCM;	/* SMN interrupt shares a line with SCM */
        ...
        os_printk(KERN_ERR "SCC: SCC went into SMN intrrupt mode\n");
	    ...
	    uint16_t * crc)
{
	...
	scc_debug("SCC: copying %ld bytes to 0x%0x.\n", count_bytes, to);
	...
	status = SCC_READ_REGISTER(SCM_ERROR_STATUS) & SCM_ACCESS_ERRORS;
	if (status != 0) {
		scc_debug("SCC copy_to_scc(): Error status detected: %08x\n",
			 status);
		/* Clear any/all bits. */
		SCC_WRITE_REGISTER(SCM_ERROR_STATUS, status);
	}
	      ...
	      uint16_t * crc)
{
	...
	uint16_t current_crc = 0;	/* local copy for fast access */
	uint32_t status;
	scc_debug("SCC: copying %ld bytes from 0x%x.\n", count_bytes, from);
	status = SCC_READ_REGISTER(SCM_ERROR_STATUS) & SCM_ACCESS_ERRORS;
	if (status != 0) {
		scc_debug
	...
	status = SCC_READ_REGISTER(SCM_ERROR_STATUS) & SCM_ACCESS_ERRORS;
	if (status != 0) {
		scc_debug("SCC copy_from_scc(): Error status detected: %08x\n",
			 status);
		/* Clear any/all bits. */
		SCC_WRITE_REGISTER(SCM_ERROR_STATUS, status);
	}
	...
	while (i-- > 0) {
		if (*--from == SCC_DRIVER_PAD_CHAR) {
			*count_bytes_stripped = SCC_BLOCK_SIZE_BYTES() - i;
			...
			scc_debug("SCC: Found non-zero interim pad: 0x%x\n",
				 *from);
			break;
		}
	}
...
static uint32_t scc_grab_config_values(void)
{
	...
	if (scc_availability != SCC_STATUS_UNIMPLEMENTED) {
		...
		scc_debug("SCC Driver: SCM config is 0x%08x\n", config_register);

		/* Get SMN status and update scc_availability */
		smn_status_register = scc_update_state();
		scc_debug("SCC Driver: SMN status is 0x%08x\n",
			 smn_status_register);
...

{
	...
	while (input_bytes_left > 0) {
		...
		scc_debug("SCC: Starting encryption. %x for %d bytes (%p/%p)\n",
	    ...
	    uint8_t * data_out, int verify_crc, unsigned long *count_out_bytes)
{
	...
	while (bytes_left && scm_error_status == 0) {
		...
		scc_debug("SCC: Starting decryption of %d bytes.\n",
			 bytes_to_copy);
	...
	if (scm_error_status == 0) {
		if (!verify_crc) {
			*count_out_bytes = bytes_copied;
			...
			scc_debug("SCC: Finishing decryption (%d bytes).\n",
				 bytes_to_copy);
...
scc_alloc_slot(uint32_t value_size_bytes, uint64_t owner_id, uint32_t * slot)
{
	...
	scc_debug("SCC: Allocating %d-byte slot for 0x%Lx\n",
	...
	if ((value_size_bytes != 0) && (value_size_bytes <= SCC_MAX_KEY_SIZE)) {
		...
		if (status != SCC_RET_OK) {
			...
			scc_debug("SCC: Allocated slot %d (0x%Lx)\n", i,
				 owner_id);
		}
	}
...
verify_slot_access(uint64_t owner_id, uint32_t slot, uint32_t access_len)
{
	...
	if ((slot < SCC_KEY_SLOTS) && scc_key_info[slot].status
		...
		status = SCC_RET_OK;
		scc_debug("SCC: Verify on slot %d succeeded\n", slot);
	} else {
		if (slot >= SCC_KEY_SLOTS) {
			scc_debug("SCC: Verify on bad slot (%d) failed\n", slot);
		} else if (scc_key_info[slot].status) {
			scc_debug("SCC: Verify on slot %d failed (%Lx) \n", slot,
				 owner_id);
		} else {
			scc_debug
...
scc_return_t scc_dealloc_slot(uint64_t owner_id, uint32_t slot)
{
	...
	if (status == SCC_RET_OK) {
		...
		scc_debug("SCC: Deallocated slot %d\n", slot);
	}
	...
	if ((status == SCC_RET_OK) && (key_data != NULL)) {
		...
		if (key_length > SCC_KEY_SLOT_SIZE) {
			...
			if (copy_to_scc(key_data,
					...
					NULL)) {
				scc_debug("SCC: RED copy_to_scc() failed for"
					 " scc_load_slot()\n");
			} else {
				if ((key_length % 4) != 0) {
					uint32_t zeros = 0;
	...
	if ((status == SCC_RET_OK) && (key_data != NULL)) {
		...
		if (key_length > SCC_KEY_SLOT_SIZE) {
			...
			if (copy_from_scc
				...
				scc_debug("SCC: RED copy_from_scc() failed for"
					 " scc_read_slot()\n");
			} else {
				status = SCC_RET_OK;
			}
			      ...
			      uint32_t length, uint8_t * black_data)
{
	...
	if (status == SCC_RET_OK) {
		...
		if (crypto_status != 0) {
			scc_debug("SCM encrypt red crypto failure: 0x%x\n",
				 crypto_status);
		} else {
			...
			if (!copy_from_scc
				...
				status = SCC_RET_OK;
				scc_debug("SCC: Encrypted slot %d\n", slot);
			}
		}
	}
			      ...
			      uint32_t length, const uint8_t * black_data)
{
	...
	if (status == SCC_RET_OK) {
		...
		if (crypto_status != 0) {
			scc_debug("SCM decrypt black crypto failure: 0x%x\n",
				 crypto_status);
		} else {
			status = SCC_RET_OK;
		}
...
static void scc_wait_completion(void)
{
	...
	scc_debug("SCC: Polled DONE %d times\n", i);
}				/* scc_wait_completion() */
