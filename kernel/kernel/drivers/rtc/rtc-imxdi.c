...
static int di_write_wait(struct rtc_drv_data *pdata, u32 val, int reg)
{
	...
	di_read(pdata, DSR);   //add by ljm clear WCF flag in DSR	
...
static irqreturn_t dryice_norm_irq(int irq, void *dev_id)
{
	...
	//printk("rtc normal irq DIER=%08x\n",dier);
...
static int dryice_rtc_probe(struct platform_device *pdev)
{
  ...
  //add rtc alarm interrupt to wake up system, xuxiaobo
  enable_irq_wake(pdata->irq);
	...
	//if (di_read(pdata, DSR) & DSR_NVF)
		//di_write_wait_err(pdata, DSR_NVF | DSR_SVF, DSR, rc, err); //non-valid flag can not be cleared 
	...
	{
		pr_alert("RTC error, no DTCMR DATA!\n");
	...
	{
		pr_alert("RTC error, no DCR_TCE Bit.\n");
		di_write_wait_err(pdata, di_read(pdata, DCR) | DCR_TCE, DCR,
				  rc, err);
	}

	/* Bypass the Oscillator, zengshu */
	/*
	if (!(di_read(pdata, DCR) & DCR_OSCB))
	{
		pr_alert("RTC warning, no DCR_OSCB, I will set it.\n");
		di_write_wait_err(pdata, di_read(pdata, DCR) | DCR_OSCB, DCR,
				rc, err);
	}
	*/
