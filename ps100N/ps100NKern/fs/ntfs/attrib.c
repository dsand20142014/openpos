...
int ntfs_map_runlist_nolock(ntfs_inode *ni, VCN vcn, ntfs_attr_search_ctx *ctx)
{
	...
	ntfs_debug("Mapping runlist part containing vcn 0x%luse it */
 ...
 * then the base inode).
 *
 * After finishing witBad flash file checksum"},
...
#define TW_STATUS_UNEXPECTED_BITS	       0x0 not equal, continue search. */
			if (rc)
				continue;
			/*
...
priearched for, i.e. if one wants to add the attribute to the
