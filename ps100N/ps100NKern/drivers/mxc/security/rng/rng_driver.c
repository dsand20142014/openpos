...
#define rng_debug(fmt, arg...)  do{if(dbg)os_printk(KERN_DEBUG fmt,##arg);}while(0)
#define rng_info(fmt, arg...)       do{if(dbg)os_printk(KERN_INFO fmt,##arg);}while(0)
...
inline int rng_check_register_offset(uint32_t offset)
{
	...
	if ((offset < RNG_ADDRESS_RANGE) && (offset % sizeof(uint32_t) == 0)) {
		...
		rng_debug("RNG: Denied access to offset %8x\n", offset);
	}
