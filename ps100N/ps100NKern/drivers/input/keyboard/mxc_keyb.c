...
#include <mach-mx25/fpga_regs.h>
#include <mach-mx25/machine.h>

//#define DEBUGFMT "keyboard(%d)-%s: "
//#define DEBUGARGS __LINE__, __FUNCTION__
//#define pr_debug(fmt, args...) {printk(KERN_ALERT DEBUGFMT fmt, DEBUGARGS, ##args);}
...
struct input_dev *mxckbd_dev = NULL;
...
//按键音定时器
static struct timer_list t_list_beep;


//按键背光定时器
static struct timer_list t_list_backlight;

extern void Buzzer_Off(void);
extern void Buzzer_On(unsigned long freq);

static void Beep_Backlight_On(void)
{
    del_timer(&t_list_beep);
    del_timer(&t_list_backlight);
    
    Buzzer_On(0);

    gpio_set_value(IOMUX_TO_GPIO(MX25_PIN_A10), 1);
    t_list_beep.expires=jiffies + (HZ/5);
    t_list_backlight.expires=jiffies + (HZ*5);
        
    add_timer(&t_list_beep);
    add_timer(&t_list_backlight);
}


static void Beep_Ctrol(unsigned long arg)
{
    Buzzer_Off();
    del_timer(&t_list_beep);
}

static void Backlight_Ctrol(unsigned long arg)
{
    gpio_set_value(IOMUX_TO_GPIO(MX25_PIN_A10), 0);
    del_timer(&t_list_backlight);
}

void mxckbd_report_key(unsigned int code, int value)
{
	input_report_key(mxckbd_dev, code, value);
...
EXPORT_SYMBOL(mxckbd_report_key);
...
{
	//Close key buzzer and key backlight
	Buzzer_Off();
	del_timer(&t_list_beep);
...
static int mxc_kpp_probe(struct platform_device *pdev)
{
	...
	if(SAND_PS4K==sand_machine())
	{
		kpp_dev.kpp_buzzer = ioremap_nocache(KEY_BUZZER,1);
		kpp_dev.kpp_light = ioremap_nocache(KEY_LIGHT,1);
	}
	else if(SAND_IPS420==sand_machine())
	{
		    mxc_request_iomux(MX25_PIN_A10, MUX_CONFIG_ALT5);
    		gpio_request(IOMUX_TO_GPIO(MX25_PIN_A10),"led_ctrol");
    		gpio_direction_output(IOMUX_TO_GPIO(MX25_PIN_A10), 1);
		    gpio_set_value(IOMUX_TO_GPIO(MX25_PIN_A10), 1);

	        t_list_beep.expires=jiffies;
	        t_list_beep.function=Beep_Ctrol;
	        init_timer(&t_list_beep);

	        t_list_backlight.expires=jiffies;
	        t_list_backlight.function=Backlight_Ctrol;
	        init_timer(&t_list_backlight);
	        t_list_backlight.expires=jiffies + (HZ*5);
	        add_timer(&t_list_backlight);
	}
