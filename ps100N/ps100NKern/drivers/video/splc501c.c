...
unsigned char HALLCD_Displaybmp(unsigned char ucFlag,unsigned char ucY,unsigned char ucX,unsigned char *aucLineDotBuf)
{
    int i = 0;
    int j = 0;
	unsigned char ucRow;
...
static int fops_splc501c_ioctl(struct inode *inode, struct file *file, unsigned int cmd,unsigned long arg)
{
	char str[34];
	int i = 0;
	char dat[10];
...
static  struct file_operations splc501c_lcd_ops = {
	.owner      = THIS_MODULE,
...
struct timer_list t_list_lcd;
#define DELTA_T 40

static void sand_splc501c_splash_fn(unsigned long arg)
{
	char argsplash[6] = {0xff,0xff,0xff,0xff,0xff,0xff};/* > */
	if(count == 0)
	{
		unsigned char chunqiu[][16] =
		{
			...
			{0x00,0x00,0x00,0x00,0x00,0xC3,0x00,0xFD,0x80,0x00,0x00,0x00,0x00,0x00,0xF0,0x00},
			{0x00,0x00,0x00,0x1F,0x00,0xBD,0x01,0xFF,0xC0,0x00,0x00,0x00,0x00,0x03,0x0C,0x00},
			{0x00,0x00,0x00,0x7F,0xC1,0x36,0x81,0xFF,0xC0,0x00,0x00,0x00,0x00,0x02,0xF4,0x00},
			{0x00,0x00,0x00,0xFF,0xE1,0x3C,0x81,0xFF,0x80,0x00,0x00,0x60,0x03,0x04,0xDA,0x00},
			{0x00,0x00,0x01,0xFF,0xF1,0x36,0x81,0xFF,0x80,0x00,0x00,0xF0,0x07,0x04,0xF2,0x00},
			{0x00,0x00,0x01,0xFF,0xF1,0x36,0x81,0xE0,0x00,0x00,0x01,0xF0,0x07,0x84,0xDA,0x00},
			{0x00,0x00,0x01,0xFB,0xF0,0x81,0x03,0xC0,0x00,0x00,0x07,0xF0,0x07,0x84,0xDA,0x00},
			{0x00,0x00,0x03,0xE0,0xF0,0xC3,0x03,0x80,0x00,0x00,0x0F,0xF0,0x0F,0x82,0x04,0x00},
			{0x00,0x00,0x03,0xE0,0xF8,0x3C,0x03,0x80,0x00,0x00,0x1E,0x00,0x0F,0x03,0x0C,0x00},
			{0x00,0x00,0x03,0xE0,0xF8,0x00,0x03,0xC0,0x00,0x00,0x3C,0xC0,0x0E,0x00,0xF0,0x00},
			{0x00,0x00,0x03,0xE0,0x78,0x00,0x07,0xE0,0x00,0x00,0x70,0xC0,0x0C,0x00,0x00,0x00},
			{0x00,0x00,0x01,0xF0,0x00,0x00,0x07,0xF0,0x00,0x01,0xE0,0xC0,0x1C,0x00,0x00,0x00},
			{0x00,0x00,0x01,0xF8,0x00,0x00,0x0F,0xF0,0x00,0x01,0xC0,0xF0,0x1F,0xC0,0x00,0x00},
			{0x00,0x00,0x01,0xFE,0x00,0x00,0x0F,0xF0,0x00,0x07,0x0F,0xFB,0x1F,0xF0,0x00,0x00},
			{0x00,0x00,0x08,0xFF,0xF8,0x00,0x0F,0xE0,0x00,0x07,0x3F,0xFE,0x18,0xF0,0x00,0x00},
			{0x00,0x00,0x1C,0xFF,0xFF,0x00,0x0F,0xC0,0x00,0x0F,0xFF,0xCE,0x38,0x70,0x00,0x00},
			{0x00,0x00,0x7C,0x7F,0xFF,0xC0,0x0F,0x80,0x00,0x0F,0xF7,0x8F,0x38,0x00,0x00,0x00},
			{0x00,0x00,0x7C,0x7F,0xFF,0xC0,0x1F,0x80,0x38,0x0E,0x07,0x87,0xF8,0x00,0x00,0x00},
			{0x00,0x00,0xFE,0x3F,0xFF,0xE0,0x1F,0xF8,0x78,0x0C,0x0F,0x83,0xF0,0x00,0x00,0x00},
			{0x00,0x00,0xFC,0x3F,0xE7,0xE0,0x1F,0xFC,0xF0,0x00,0x1F,0x00,0xF0,0x00,0x00,0x00},
			{0x00,0x00,0xF8,0x3F,0x83,0xE0,0x1F,0xFC,0xF0,0x00,0x3F,0x01,0xFC,0x00,0x00,0x00},
			{0x00,0x00,0xF0,0x7F,0x01,0xE0,0x1F,0x03,0xF0,0x00,0x7B,0x01,0xEF,0x00,0x00,0x00},
			{0x00,0x00,0xF0,0x7F,0x01,0xE0,0x1F,0x07,0xFC,0x01,0xF3,0x03,0xC3,0xC0,0x00,0x00},
			{0x00,0x00,0xF8,0x7E,0x01,0xE0,0x1F,0xFF,0x04,0x03,0xF3,0x07,0x81,0xF0,0x00,0x00},
			{0x00,0x00,0xFC,0xFC,0x03,0xE0,0x3F,0xFA,0x00,0x03,0xC3,0x0F,0x00,0x7C,0x00,0x00},
			{0x00,0x00,0xFF,0xFC,0xFF,0xE0,0x7F,0xE0,0x00,0x01,0x83,0x1C,0x00,0x3E,0x00,0x00},
			{0x00,0x00,0xFF,0xF8,0xFF,0xE0,0xFF,0xFF,0xF0,0x00,0x03,0x80,0x00,0x1F,0x80,0x00},
			{0x00,0x00,0x7F,0xF1,0xFF,0xC1,0xFB,0xF1,0xF8,0x00,0x03,0x00,0x00,0x0F,0x80,0x00},
			{0x00,0x00,0x3F,0xF1,0xFF,0x81,0xE3,0xC1,0xF8,0x00,0x03,0x00,0x00,0x07,0x80,0x00},
			{0x00,0x00,0x1F,0xC0,0x7F,0x00,0x03,0xFF,0x30,0x00,0x00,0x00,0x00,0x02,0x00,0x00},
			{0x00,0x00,0x02,0x00,0x1C,0x00,0x00,0x3C,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00},
			{0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00},
			{0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00},
			{0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00},
			{0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00},
		...
		HALLCD_Displaybmp(0, 0, 0,chunqiu);
	}
	if(count<=20)
	{
		if((count%21) == 0)
		{
			HALLCD_ClearLine(7 - 7);
		}
		HALLCD_Displayascii5x7(0, 7, count%21, argsplash);
		t_list_lcd.expires+=DELTA_T;
		add_timer(&t_list_lcd);
		count++;
		return;
	}
}

static void begin_splash(void)
{
	count=0;
	init_timer(&t_list_lcd);
	t_list_lcd.expires=jiffies;
	t_list_lcd.function=sand_splc501c_splash_fn;
	add_timer(&t_list_lcd);
}
/******************splash end*************************/
static struct class *splc501c_class;

static int splc_probe (struct platform_device *pdev)
{
	int ret;
	...
	HALLCD_Init();
	printk("LCD_SPLC501C Init success.\n");

    //keyboard backlight on
    mxc_request_iomux(MX25_PIN_A10, MUX_CONFIG_GPIO);
    gpio_set_value(IOMUX_TO_GPIO(MX25_PIN_A10), 1);
    gpio_direction_output(IOMUX_TO_GPIO(MX25_PIN_A10), 1);
	...
	if(ret <0)
	{
		printk("SPLC501C can't get major number\n");
		return ret;
	}
...
static int  splc_remove(struct platform_device *pdev)
{
...
static void mxc_splc_release(struct device * dev)
{
    return ;
}

static int  splc_suspend(struct platform_device *dev, pm_message_t state)
{
    //display off
    HALLCD_WriteCMDChip(0xAE);

    //lcd power off
    HALLCD_WriteCMDChip(0x28);
    ...
    printk("lcd splc501 suspended\n");

    return 0;
}

static int  splc_resume(struct platform_device *dev)
{
    //lcd power on
    HALLCD_WriteCMDChip(0x2F);

    //display on
    HALLCD_WriteCMDChip(0xAF);
    ...
    printk("lcd splc501 resume success\n");

    return 0;
}


static void  splc_shutdown(struct platform_device *dev)
{
    //display all off
    HALLCD_WriteCMDChip(0xA4);
    ...
    printk("lcd splc501 poweroff\n");
}

static struct platform_device splc_device ={
                .name = SPLC_NAME,
                .id   = -1,
                .dev  = {
                                .release  =  mxc_splc_release,
                },
};

static struct platform_driver  splc_driver ={
                .suspend         = splc_suspend,
                .resume          = splc_resume,
                .probe           = splc_probe,
                .remove          = splc_remove,
                .shutdown        = splc_shutdown,
                .driver = {
                                .name   = SPLC_NAME,
                                .owner  = THIS_MODULE,
                                },
...
static int __init imx258_splc_init(void)
{
    platform_device_register(&splc_device);
    platform_driver_register(&splc_driver );

    return 0;
}

static void __exit imx258_splc_exit(void)
{
    platform_driver_unregister(&splc_driver );
    platform_device_unregister(&splc_device);
}

module_init(imx258_splc_init);
module_exit(imx258_splc_exit);
