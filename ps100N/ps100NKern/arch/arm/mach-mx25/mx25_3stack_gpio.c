...
#define GPIO_INPUT_CFG ( PAD_CTL_DRV_NORMAL | PAD_CTL_ODE_CMOS | PAD_CTL_100K_PU | PAD_CTL_PKE_ENABLE | PAD_CTL_PUE_PULL)

static struct mxc_iomux_pin_cfg __initdata mxc_iomux_pins[] = {
	{	MX25_PIN_GPIO_F, MUX_CONFIG_FUNC,GPIO_INPUT_CFG,             },
	{	MX25_PIN_A25, MUX_CONFIG_GPIO,	VSTBYACK_4GPIO_CTL,	         },
};
...
void gpio258_buttonpower_init(void)
{
	//mxc_request_iomux(MX25_PIN_A25, MUX_CONFIG_GPIO);
	//mxc_iomux_set_pad(MX25_PIN_A25, VSTBYACK_4GPIO_CTL);
	gpio_request(IOMUX_TO_GPIO(MX25_PIN_A25),NULL);
	gpio_direction_output(IOMUX_TO_GPIO(MX25_PIN_A25), 0);
}
EXPORT_SYMBOL(gpio258_buttonpower_init);

int gpio258_getadcpwpin(void)
{
	gpio_request(IOMUX_TO_GPIO(MX25_PIN_GPIO_F),NULL);
	gpio_direction_input(IOMUX_TO_GPIO(MX25_PIN_GPIO_F));
	return	gpio_get_value(IOMUX_TO_GPIO(MX25_PIN_GPIO_F));	 
}
EXPORT_SYMBOL(gpio258_getadcpwpin);

int gpio258_getwifipwpin(void)
{
	return 0;
}
EXPORT_SYMBOL(gpio258_getwifipwpin);

void gpio258_buttonpower_csup(void)
{
	gpio_set_value(IOMUX_TO_GPIO(MX25_PIN_A25), 1);
	msleep(1);//We need to give a little time for charge the 1nF C;
}
EXPORT_SYMBOL(gpio258_buttonpower_csup);

void gpio258_buttonpower_csdown(void)
{
	gpio_set_value(IOMUX_TO_GPIO(MX25_PIN_A25), 0);
}
EXPORT_SYMBOL(gpio258_buttonpower_csdown);

void gpio258_led_init(void)
{
	//LED_1
	...
	//LED_2
	mxc_request_iomux(MX25_PIN_A18, MUX_CONFIG_GPIO);
	mxc_iomux_set_pad(MX25_PIN_A18, VSTBYACK_4GPIO_CTL);
	gpio_request(IOMUX_TO_GPIO(MX25_PIN_A18),NULL);
	gpio_direction_output(IOMUX_TO_GPIO(MX25_PIN_A18), 1);

	//LED_3
	mxc_request_iomux(MX25_PIN_A19, MUX_CONFIG_GPIO);
	mxc_iomux_set_pad(MX25_PIN_A19, VSTBYACK_4GPIO_CTL);
	gpio_request(IOMUX_TO_GPIO(MX25_PIN_A19),NULL);
	gpio_direction_output(IOMUX_TO_GPIO(MX25_PIN_A19), 1);

	//LED_4
	mxc_request_iomux(MX25_PIN_A24, MUX_CONFIG_GPIO);
	mxc_iomux_set_pad(MX25_PIN_A24, VSTBYACK_4GPIO_CTL);
	gpio_request(IOMUX_TO_GPIO(MX25_PIN_A24),NULL);
	gpio_direction_output(IOMUX_TO_GPIO(MX25_PIN_A24), 1);

	//CPLD
	//LED_4
	mxc_request_iomux(MX25_PIN_A17, MUX_CONFIG_GPIO);
	mxc_iomux_set_pad(MX25_PIN_A17, VSTBYACK_4GPIO_CTL);
	gpio_request(IOMUX_TO_GPIO(MX25_PIN_A17),NULL);
	gpio_direction_output(IOMUX_TO_GPIO(MX25_PIN_A17), 0);


}
EXPORT_SYMBOL(gpio258_led_init);
...
void gpio_uart_active(int port, int no_irda)
{
	...
	case 4:
		...
		if((SAND_PS4K==sand_machine())||(SAND_PS4KV2==sand_machine()))
		{
...
void gpio_uart_inactive(int port, int no_irda)
{
	...
	case 4:
		...
		{
			gpio_request(IOMUX_TO_GPIO(MX25_PIN_LBA), NULL);
			gpio_request(IOMUX_TO_GPIO(MX25_PIN_ECB), NULL);
...
void gpio_keypad_active(void)
{
	...
	mxc_request_iomux(MX25_PIN_KPP_COL4, MUX_CONFIG_ALT3);
...
#define KPP_PAD_CTL_COL (PAD_CTL_PKE_ENABLE | PAD_CTL_PUE_PUD | \
	...
	mxc_iomux_set_pad(MX25_PIN_KPP_COL4, KPP_PAD_CTL_COL);
...
void gpio_keypad_inactive(void)
{
	...
	gpio_request(IOMUX_TO_GPIO(MX25_PIN_KPP_COL4), NULL);
	...
	mxc_free_iomux(MX25_PIN_KPP_COL4, MUX_CONFIG_GPIO);
}
...
#define I2C_PAD_CTL ( PAD_CTL_PKE_NONE | PAD_CTL_ODE_OpenDrain)
...
//	mxc_request_iomux(MX25_PIN_CONTRAST, MUX_CONFIG_FUNC);
	...
	if(SAND_PS4KV2==sand_machine())
	{
		mxc_request_iomux(MX25_PIN_FEC_MDC, MUX_CONFIG_ALT7);
		mxc_request_iomux(MX25_PIN_FEC_MDIO, MUX_CONFIG_ALT7);
	}
	else
	{
...
//	mxc_iomux_set_pad(MX25_PIN_CONTRAST, LCD_PAD_CTL);
	...
	if(SAND_PS4KV2==sand_machine())
	{
		mxc_iomux_set_pad(MX25_PIN_FEC_MDC, LCD_PAD_CTL);
		mxc_iomux_set_pad(MX25_PIN_FEC_MDIO, LCD_PAD_CTL);
	}
	else
	{
...
//	gpio_request(IOMUX_TO_GPIO(MX25_PIN_GPIO_F), NULL); /*D17*/
	...
	{
		gpio_request(IOMUX_TO_GPIO(MX25_PIN_D14), NULL);
		gpio_request(IOMUX_TO_GPIO(MX25_PIN_D15), NULL);
	}
...
#define SDHC_PAD_CTL (PAD_CTL_PKE_ENABLE | PAD_CTL_PUE_PUD | PAD_CTL_SRE_FAST)
...
//		mxc_request_iomux(MX25_PIN_A15, MUX_CONFIG_ALT5); /*SD1_DET*/
		...
		{
			mxc_request_iomux(MX25_PIN_D8, MUX_CONFIG_ALT5); /*SD1_DET*/
		}
		else
		{
			mxc_request_iomux(MX25_PIN_D8, MUX_CONFIG_ALT5); /*SD1_WP*/
			mxc_request_iomux(MX25_PIN_D9, MUX_CONFIG_ALT5); /*SD1_DET*/
		}
...
//		mxc_iomux_set_pad(MX25_PIN_A15, PAD_CTL_DRV_NORMAL);
		if(sand_machine()==SAND_PS4KV2)
		{
			mxc_iomux_set_pad(MX25_PIN_CSI_D8, PAD_CTL_PKE_ENABLE | PAD_CTL_PUE_PULL | PAD_CTL_22K_PU); 
		}
		else
		{
			mxc_iomux_set_pad(MX25_PIN_D8, PAD_CTL_PKE_ENABLE | PAD_CTL_PUE_PULL | PAD_CTL_22K_PU); 
			mxc_iomux_set_pad(MX25_PIN_D9, PAD_CTL_PKE_ENABLE | PAD_CTL_PUE_PULL | PAD_CTL_22K_PU);
		}
		...
		if(sand_machine()==SAND_PS4KV2)
		{
			...
			gpio_request(IOMUX_TO_GPIO(MX25_PIN_CSI_D8), "sd_det");
			gpio_direction_input(IOMUX_TO_GPIO(MX25_PIN_CSI_D8)); /*SD1_DET*/
		}
		else
		{		
			gpio_request(IOMUX_TO_GPIO(MX25_PIN_D8), "sd_wp");
			gpio_request(IOMUX_TO_GPIO(MX25_PIN_D9), "sd_det");
			gpio_direction_input(IOMUX_TO_GPIO(MX25_PIN_D8)); /*SD1_WP*/
			gpio_direction_input(IOMUX_TO_GPIO(MX25_PIN_D9)); /*SD1_DET*/
		}
		break;
	case 1:
		/* SDHC2 */
...
//	ret = gpio_get_value(IOMUX_TO_GPIO(MX25_PIN_A15));
	...
	{
		ret = gpio_get_value(IOMUX_TO_GPIO(MX25_PIN_D9));
	}
	else if(sand_machine()==SAND_PS4KV2)
	{
		ret = gpio_get_value(IOMUX_TO_GPIO(MX25_PIN_CSI_D8));
	}
	else
	{
		ret = gpio_get_value(IOMUX_TO_GPIO(MX25_PIN_D9));
...
//	rc = gpio_get_value(IOMUX_TO_GPIO(MX25_PIN_A14));
	...
	{
		rc = gpio_get_value(IOMUX_TO_GPIO(MX25_PIN_D8));
	}
	return rc;
}
...
void gpio_sensor_active(void)
{
	...
	if(SAND_DEMOBOARD==sand_machine()) {		//On the sand demoboard, we are using CPLD instead of gpio.zengshu.
		gpio_request(IOMUX_TO_GPIO(MX25_PIN_UPLL_BYPCLK), "csi_rst");
		gpio_direction_output(IOMUX_TO_GPIO(MX25_PIN_UPLL_BYPCLK), 0);
		gpio_set_value(IOMUX_TO_GPIO(MX25_PIN_UPLL_BYPCLK), 0);	
		mdelay(20);
		gpio_set_value(IOMUX_TO_GPIO(MX25_PIN_UPLL_BYPCLK), 1);
		
		//u8 * csi_cpld_addr=ioremap_nocache(0xb2d00000,1);
		//iowrite8(0x80,csi_cpld_addr);
		//pr_alert(">>>>>>>>>>>>>>>>>1:data in csi_cpld_addr is %x\n",ioread8(csi_cpld_addr));
		//mdelay(20);
		//iowrite8(0x80|0x40,csi_cpld_addr);
		//pr_alert(">>>>>>>>>>>>>>>>>2:data in csi_cpld_addr is %x\n",ioread8(csi_cpld_addr));
		//iounmap(csi_cpld_addr);
	} else if(SAND_IPS420){
				u8 reg_tmp;
...
void gpio_sensor_inactive(void)
{
	...
	if(SAND_DEMOBOARD==sand_machine()) { //On the sand demoboard, we are using CPLD instead gpio.zengshu.
		mxc_free_iomux(MX25_PIN_UPLL_BYPCLK, MUX_CONFIG_GPIO);
		//u8 * csi_cpld_addr=ioremap_nocache(0xb2d00000,1);
        	//iowrite8(0x00,csi_cpld_addr);
		//pr_alert(">>>>>>>>>>>>>>>>>3:data in csi_cpld_addr is %x\n",ioread8(csi_cpld_addr));
        	//iounmap(csi_cpld_addr);
...
void gpio_actvite_sand_cpld(void)
{
	mxc_request_iomux(MX25_PIN_UPLL_BYPCLK,MUX_CONFIG_ALT5);
	mxc_iomux_set_pad(MX25_PIN_UPLL_BYPCLK, PAD_CTL_DRV_3_3V);
	gpio_set_value(IOMUX_TO_GPIO(MX25_PIN_UPLL_BYPCLK), 1);
}
EXPORT_SYMBOL(gpio_actvite_sand_cpld);
