...
#define FEC_MII_ST      0x40000000  //(0x1 << 30)
...
#define FEC_MII_TA      0x00020000  //(0x2 << 16)
...
//	printf("%s,%d, val:0x%x\n", __func__,__LINE__, val);
	...
	__fec_mii_read(fecp, addr, 31, &val);
//	printf("####%s,%d, 31 val:0x%x\n", __func__,__LINE__, val);

	__fec_mii_read(fecp, addr, 31, &val);
	//printf("#######%s,%d,31 val:0x%x\n", __func__,__LINE__, val);

	__fec_mii_read(fecp, addr, 18, &val);
	//	printf("#######%s,%d, 18 val:0x%x\n", __func__,__LINE__, val);
...
#if 0
#define IOMUXGPR  0x43FAC000
	printf("Show FEC_TX_CLK  MUX\n");
	u32 mux_reg = IOMUXGPR + 0x1e8;
	u32 mux_val = __REG(mux_reg);
    printf("mux_val:0x%x\n", mux_val);

    printf("Show FEC_TX_CLK PAD\n");
	u32 pad_reg = IOMUXGPR + 0x3e0;
	u32 pad_val = __REG(pad_reg);
    printf("mux_val:0x%x\n", pad_val);
#endif
...
//	printf("IN %s\n",__func__);
	...
	for (i = 0; i < sizeof(fec_info) / sizeof(fec_info[0]); i++) {
		...
		dev =
		    (struct eth_device *)memalign(CONFIG_SYS_CACHELINE_SIZE, sizeof *dev);
		if (dev == NULL)
			hang();
