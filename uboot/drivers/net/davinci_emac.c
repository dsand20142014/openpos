...
static void davinci_eth_mdio_enable(void)
{
	...
	printf("davinci_eth_mdio_enable\n");

	clkdiv = (EMAC_MDIO_BUS_FREQ / EMAC_MDIO_CLOCK_FREQ) - 1;

	printf("davinci_eth_mdio_enable:105\n");

	writel((clkdiv & 0xff) | MDIO_CONTROL_ENABLE | MDIO_CONTROL_FAULT | MDIO_CONTROL_FAULT_ENABLE, &adap_mdio->CONTROL);
...
#endif
	printf("davinci_eth_mdio_enable:112\n");

	while (readl(adap_mdio->CONTROL) & MDIO_CONTROL_IDLE) {
		printf("davinci_eth_mdio_enable:readl(adap_mdio->CONTROL):0x%x\n", readl(adap_mdio->CONTROL));
	}
	printf("davinci_eth_mdio_enable:115\n");
}
...
//	sprintf(phy.name, "DP83848 @ 0x%02x", active_phy_addr);
/*	int	phy_addr = 0x1;
	dp83848_init_phy(phy_addr);
	dp83848_is_phy_connected(phy_addr);
	dp83848_get_link_speed(phy_addr);
	dp83848_auto_negotiate(phy_addr);
*/
#if 0
