...
{
	int CPLD_POWER_REG =	0xa0300000;
	__REG(CPLD_POWER_REG)|=0x80;
}
#endif
...
#if 0
	{
			...
			do{
				...
				__REG(UART_PHYS1 + 0x40) = 0xA5;

					/* wait for transmitter to be ready */
				while (!(__REG(UART_PHYS1 + 0xb4) & (1<<6)))
						;

				__REG(UART_PHYS2 + 0x40) = 0xA5;

						/* wait for transmitter to be ready */
					while (!(__REG(UART_PHYS2 + 0xb4) & (1<<6)))
							;
				}while(1);
		}
#endif
