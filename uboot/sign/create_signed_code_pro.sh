...
#create a sha256+1024bit-RSA sinature for csf_pro.bin
...
echo "Creating the signed u-boot => $1_rsa.bin"
rdb_size=`ls -all $1_signed.bin | gawk '{ print $5 }'`
# calculate the padding size.
padding_size=`expr $cert_offset - $rdb_size`

# create a zero padding file whose size is padding_size
dd if=/dev/zero of=zero_file.tmp bs=1 count=$padding_size &> /dev/null
cat $1_signed.bin zero_file.tmp  cert.crt> $1_rsa.bin
