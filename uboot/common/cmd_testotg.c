...
void imx_udc_reset(struct imx_udc_struct *imx_usb)
{
	...
	do{}while(readl(imx_usb->base + USB_ADAR) & DADR_CFG);
}
#endif

int do_otgtest(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	 udc_init();
}
#if 0
...
U_BOOT_CMD (otg_test, 6, 1, do_otgtest,
	    "otg_test",
