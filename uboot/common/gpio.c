...
/*
 * Get a GPIO pin's data output
 * @param port		number of gpio port
 * @param index		gpio pin index value (0~31)
 * @returnvalue		value to be got (only 0 or 1 is valid)
 */
static u32 _get_gpio_dataout(u32 port, u32 index)
{
	u32 reg = gpio_port[port].base + DR;
	u32 l = 0;
...
/*!
 * Exported function to get a GPIO pin's data output
 * @param pin		a name defined by \b iomux_pin_name_t
 * @returnvalue		value to be got (only 0 or 1 is valid)
 */

u32 mxc_get_gpio_dataout(iomux_pin_name_t pin)
{
	u32 port;
	u32 gpio = IOMUX_TO_GPIO(pin);

	port = GPIO_TO_PORT(gpio);
	return _get_gpio_dataout(port, GPIO_TO_INDEX(gpio));
}