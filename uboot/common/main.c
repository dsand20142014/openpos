...
//0:normal boot. 1:recovery boot.
static __inline__ int bootmode_test(void)
{
    struct kpp_key_info key_info = {0, 0};
    mxc_kpp_init();
    udelay(1000);
    if (mxc_kpp_getc(&key_info)) {
                if (TEST_CANCEL_KEY_DEPRESS(key_info.val, key_info.evt)) {
...
#endif /* CONFIG_BOOTCOUNT_LIMIT */
    switch(bootmode_test())
    {
        case 1:
            s = getenv ("bootcmd_cramfs");
            break;
        default:
            s = getenv ("bootcmd");
            break;
