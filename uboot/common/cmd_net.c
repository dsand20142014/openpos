...
//#endif
	...
	filesize = 0;
	//filesize = *(unsigned  int *)(load_addr);
	filesize = size -256;
	printf("\nfilesize=%ld \n", filesize);        
       printf("\n%ld bytes read\n", size); 
	/*if(filesize > 0x10000000)
	{
		...
		for(i=0;i<size;i++)
			*((unsigned char *)(load_addr + i)) = 0;  //clear loaded data
		return 1;
	}*/
        //rsaresult = RSA_recovery_check((unsigned char *)(load_addr + 4), filesize);
        rsaresult = RSA_recovery_check((unsigned char *)(load_addr ), filesize);
	if(rsaresult)
	{
