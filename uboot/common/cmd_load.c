...
//#ifdef PCI_CHECK
	//set_kerm_bin_mode ((ulong *) (offset-4));
//#else
...
//#endif	
	...
	if(rsaresult)
	{
		...
		for(j=0;j<size;j++)
			//*((unsigned char *)(offset -4 + j)) = 0;  //clear loaded data
			*((unsigned char *)(offset + j)) = 0;  //clear loaded data
		size = 0;
		offset = 0;
	}
	else
