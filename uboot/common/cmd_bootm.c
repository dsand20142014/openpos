...
extern unsigned char  RSA_recovery_check(unsigned char *pucBegin, unsigned long ulFilesize);
/*
...
void hab_status_read(void)
{  
	unsigned int uiStatus;  
	unsigned int i;   
	unsigned char fuse_value;     
	printf("HAB_status read!\n");  
	uiStatus = *((unsigned int *)0x780018d4);   
	printf("HAB_status=%08x\n",uiStatus);   
	/*for(i=1;i<32;i++)   	
	{   	  
		fuse_value =*((unsigned char *)0x53FF0000+0x1000+(4*i));	   
		printf("fuse2_%d=%02x\n",4*i,fuse_value);   	
	}  */ 
	
	fuse_value =*((unsigned char *)0x53FF0000+0x800+0x10);     
	printf("fuse0_10=%02x\n",fuse_value);
	fuse_value =*((unsigned char *)0x53FF0000+0x800+0x50);     
	printf("fuse0_50=%02x\n",fuse_value);
...
int image_check_rsa(image_header_t *hdr)
{
      ulong data = image_get_data (hdr);
      ulong len = image_get_data_size (hdr);
    //  unsigned char hash_value[20];
     // unsigned char rsa_recover[128];
      //unsigned char hash_value_rec[20];
     // int i;
      ulong hashlen;
      unsigned char rsa_result;

	hashlen = sizeof(image_header_t)+len;
	 printf("hashlen0=%08x\n",hashlen);
	if(hashlen % 4)
	{
		hashlen +=4 - (hashlen % 4);
	 ...
	 printf("hashlen1=%08x\n",hashlen);
	 rsa_result = RSA_recovery_check((unsigned char *)hdr,hashlen);
	 return rsa_result;
       /*imx_hash_compute(1,(unsigned int)hdr,hashlen,hash_value);
	for(i=0;i<20;i++)
	    printf("hash0[%d]=%02x/",i,hash_value[i]);
	 RSA_PublicDecrypt(ucPubKey_modulus,128,ucPubkey_Exp,3,
					(unsigned char *)(data+len),128,
					rsa_recover);
        for(i=108;i<128;i++)
		printf("rsa_recov[%d]=%02x/",i,rsa_recover[i]);
	for(i=0;i<20;i++)
		hash_value_rec[i]=rsa_recover[108+i];
	if(!memcmp(hash_value,hash_value_rec,20))
		return 0;
	else
		return 1;*/
}
...
#ifdef PCI_CHECK
	if(!image_check_rsa(hdr))
	   	printf("RSA verify ok!\n");
	else
	{
	   	  printf("rsa verify fail!\n");
	   	  return NULL;
	}
#endif
